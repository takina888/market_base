#!/usr/bin/env python3
"""Static regression audit for MARKET BASE V333.14 cache/refresh stabilization."""
...
def main() -> int:
    ...
    for page in html_files:
        ...
        if controllers:
            ...
            if not path_ends(controller, "assets/js/market-base-update-controller-v334.js"):
    ...
    check(not bad_build_meta, "all HTML market-base-build metadata is V333.14", failures)
    if bad_build_meta:
        failures.extend(bad_build_meta)
    check(not bad_site_meta, "all optional market-base-site-build metadata is V333.14", failures)
    if bad_site_meta:
    ...
    check(not stale_refs, "HTML boot references use only V333.14 physical files/tokens", failures)
    if stale_refs:
        failures.extend(stale_refs)

    version = (ROOT / "version.txt").read_text(encoding="utf-8").strip()
    check(version == BUILD_ID, "version.txt matches V333.14 build ID", failures)

    manifest = json.loads((ROOT / "manifest.json").read_text(encoding="utf-8"))
    check(manifest.get("version") == "V333.14", "manifest version is V333.14", failures)
    ...
    check(TOKEN in manifest.get("start_url", "") and TOKEN in manifest.get("id", ""),
          "manifest start_url and id use V333.14 token", failures)

    canonical = {
        "assets/js/market-base-build-v334.js": [BUILD_ID, TOKEN],
        "assets/js/market-base-runtime-v334.js": [BUILD_ID],
        "assets/js/market-base-scroll-controls-v334.js": [TOKEN, "scheduleUpdateControllerFallback();"],
        "assets/js/market-base-update-controller-v334.js": [BUILD_ID, "controllerRevision: 'v334'", "pageShellIsCurrent", "AUTO_REFRESH_STATE_KEY"],
        "assets/js/market-base-offline-manifest-v334.js": ["MARKET_BASE_OFFLINE_MANIFEST_V333_14_CACHE_REFRESH_STABILIZATION_20260804"],
        "settings/assets/offline-settings-v334.js": [BUILD_ID],
    }
    ...
    runtime = (ROOT / "assets/js/market-base-runtime-v334.js").read_text(encoding="utf-8")
    ...
    offline_settings = (ROOT / "settings/assets/offline-settings-v334.js").read_text(encoding="utf-8")
    ...
    compatibility_pairs = [
        ("assets/js/market-base-build.js", "assets/js/market-base-build-v334.js"),
        ("assets/js/market-base-runtime-r11348.js", "assets/js/market-base-runtime-v334.js"),
        ("assets/js/market-base-scroll-controls-r11328.js", "assets/js/market-base-scroll-controls-v334.js"),
        ("assets/js/market-base-offline-manifest-v324.js", "assets/js/market-base-offline-manifest-v334.js"),
        ("settings/assets/offline-settings.js", "settings/assets/offline-settings-v334.js"),
    ]
    ...
    for legacy in [
        ...
        check("market-base-update-controller-v334.js" in text and TOKEN in text,
              f"legacy controller shim migrates to V333.14: {legacy}", failures)

    sw_path = ROOT / "sw.js"
    sw = sw_path.read_text(encoding="utf-8")
    check(f"const BUILD_ID='{BUILD_ID}'" in sw, "Service Worker build ID is V333.14", failures)
    check("market-base-offline-manifest-v334.js" in sw and TOKEN in sw.splitlines()[1],
    ...
    js_to_check += [ROOT / f"assets/js/market-base-update-controller-v{version}.js" for version in (322, 331, 332, 333)]
    ...
    if failures:
        ...
        for failure in failures:
            print(f" - {failure}")
        return 1
    print("PASSED: V333.14 cache/refresh static audit")
    return 0
