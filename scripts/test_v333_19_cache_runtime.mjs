...
const BUILD='MARKET_BASE_V333_19_JFM_V077_CURRENCY_HF3_20260907';
...
// lazy current-token asset to use the network. ACK is sent only after put().
{
  const shared=new CacheStorageMock();
  const runtime=await shared.open(RUNTIME);
  const repairCache=await shared.open('mb-update-state-v333-19');
...
console.log('V333.19 cache/runtime lifecycle: PASS');
