#!/usr/bin/env python3
"""Static release-integrity audit for MARKET BASE V333.18.
...
BUILD_ID = "MARKET_BASE_V333_18_CACHE_RADIO_NAVIGATION_STABILITY_20260810"
TOKEN = "20260810-v333-18-cache-radio-navigation-stability"
VERSION = "V333.18"
EXPECTED_HTML = 38
...
def main() -> int:
    ...
    check(not metadata_errors, "all public HTML has exact V333.18 build/site metadata", metadata_errors)
    check(
    ...
    check(not token_errors, "all local HTML v/mbv cache queries use the V333.18 token", token_errors)
    check(not stale_release_errors, "no V333.16/V333.17 build or token remains in public HTML", stale_release_errors)
    check(
        len(current_shell_pages) == 12 and not legacy_live_asset_errors,
        "12 shell pages use V333.18 JavaScript and no public HTML references old app/shell JavaScript",
        [f"current shell page count={len(current_shell_pages)}", *legacy_live_asset_errors],
    )

    check(read("version.txt").strip() == BUILD_ID, "version.txt matches V333.18 BUILD_ID")
    try:
        web_manifest = json.loads(read("manifest.json"))
    except (OSError, json.JSONDecodeError) as error:
        web_manifest = {}
        failures.append(f"manifest.json could not be parsed: {error}")
    check(web_manifest.get("version") == VERSION, "manifest version is V333.18")
    check(web_manifest.get("build_id") == BUILD_ID, "manifest build_id matches V333.18")
    check(
        TOKEN in str(web_manifest.get("start_url", ""))
        and TOKEN in str(web_manifest.get("id", "")),
        "manifest start_url and id use the V333.18 token",
    )
    ...
    check(not missing_assets, "all V333.18 navigation/performance assets exist", missing_assets)
    ...
    check(not missing_index_refs, "index.html references every V333.18 asset", missing_index_refs)
    check(not stale_index_refs, "index.html omits old app and old shell JavaScript", stale_index_refs)

    sw = read("sw.js")
    check(BUILD_ID in sw and TOKEN in sw, "Service Worker declares the V333.18 generation")
    ...
    check(
        ...
        "offline manifest metadata matches V333.18",
    )
    ...
    check(not missing_offline, "offline manifest includes every V333.18 asset", missing_offline)
    ...
    if failures:
        ...
        for failure in failures:
            print(f" - {failure}")
        return 1
    print("PASSED: V333.18 release integrity")
    return 0
