#!/usr/bin/env python3
"""Static release-integrity audit for MARKET BASE V333.19.
...
CURRENT_APP = "assets/js/app-v273-country-profile-r28-refresh-route-header-r99.js"
CURRENT_NAVIGATION = "assets/js/market-base-navigation-v333-18.js"
CURRENT_DEFERRED = "assets/js/market-base-home-deferred-v333-18.js"
CURRENT_SHELL = "assets/js/market-base-pc-unified-shell-v333-18.js"
CURRENT_NAVIGATION_CSS = "assets/css/market-base-navigation-v333-18.css"
CURRENT_INSTALL_HELPER = "assets/js/market-base-install-helper-v333-19.js"
CURRENT_INSTALL_CSS = "assets/css/market-base-install-helper-v333-19.css"
CURRENT_CONTROLLER = "assets/js/market-base-update-controller-v335.js"
STABLE_PWA_ID = "./?v=20260803-v333-10-cloudflare-web-analytics"
...
def main() -> int:
    ...
    html_files = sorted(
        ...
        if "HANDOFF_DOCUMENTS" not in path.parts and "android" not in path.parts
    )
    ...
    for page in html_files:
        ...
        for _, url in parser.urls:
            ...
            if (
                "market-base-pc-unified-shell-r95-v1.js" in asset_path
                or "market-base-pc-unified-shell-v333-17.js" in asset_path
            ):
                legacy_live_asset_errors.append(f"{relative}: old shell {url!r}")
            if re.search(
                r"app-v273-country-profile-r28-refresh-route-header-r(?:95|96|97|98)\.js$",
                asset_path,
            ):
                legacy_live_asset_errors.append(f"{relative}: old app {url!r}")
        stale_markers = (
            "MARKET_BASE_V333_16_RADIO_LIFECYCLE_UI_PERFORMANCE_20260810",
            "20260810-v333-16-radio-lifecycle-ui-performance",
            "MARKET_BASE_V333_17_CACHE_COHERENCE_SMOOTH_NAVIGATION_20260810",
            "20260810-v333-17-cache-coherence-smooth-navigation",
            "MARKET_BASE_V333_18_CACHE_RADIO_NAVIGATION_STABILITY_20260810",
            "20260810-v333-18-cache-radio-navigation-stability",
        )
        for marker in stale_markers:
            if marker in body:
                stale_release_errors.append(f"{relative}: stale release marker {marker}")

    check(not metadata_errors, "all public HTML has exact V333.19 build/site metadata", metadata_errors)
    check(
    ...
    check(not token_errors, "all local HTML v/mbv cache queries use the V333.19 token", token_errors)
    check(not stale_release_errors, "no V333.16/V333.17/V333.18 build or token remains in public HTML", stale_release_errors)
    check(
        len(current_shell_pages) == 12 and not legacy_live_asset_errors,
        "12 shell pages use V333.19 JavaScript and no public HTML references old app/shell JavaScript",
        [f"current shell page count={len(current_shell_pages)}", *legacy_live_asset_errors],
    )

    check(read("version.txt").strip() == BUILD_ID, "version.txt matches V333.19 BUILD_ID")
    try:
        web_manifest = json.loads(read("manifest.json"))
    except (OSError, json.JSONDecodeError) as error:
        web_manifest = {}
        failures.append(f"manifest.json could not be parsed: {error}")
    check(web_manifest.get("version") == VERSION, "manifest version is V333.19")
    check(web_manifest.get("build_id") == BUILD_ID, "manifest build_id matches V333.19")
    check(
        TOKEN in str(web_manifest.get("start_url", ""))
        and web_manifest.get("id") == STABLE_PWA_ID,
        "manifest start_url uses V333.19 while PWA id preserves the public V333.10 identity",
    )
    ...
    current_assets = [
        ...
        CURRENT_INSTALL_HELPER,
        CURRENT_INSTALL_CSS,
    ]
    missing_assets = [relative for relative in current_assets if not (ROOT / relative).is_file()]
    check(not missing_assets, "all V333.19 navigation/performance assets exist", missing_assets)
    ...
    stale_index_refs = [
        ...
        if re.search(r"app-v273-country-profile-r28-refresh-route-header-r(?:95|96|97|98)\.js", url)
        or "market-base-pc-unified-shell-r95-v1.js" in url
        or "market-base-pc-unified-shell-v333-17.js" in url
    ]
    check(not missing_index_refs, "index.html references every V333.19 asset", missing_index_refs)
    check(not stale_index_refs, "index.html omits old app and old shell JavaScript", stale_index_refs)

    sw = read("sw.js")
    check(BUILD_ID in sw and TOKEN in sw, "Service Worker declares the V333.19 generation")
    ...
    check(not missing_offline, "offline manifest includes every V333.19 asset", missing_offline)

    obsolete_patterns = [
        re.compile(r"^\./assets/js/app-v273-country-profile-r28-refresh-route-header-r(?:95|96|97|98)\.js$"),
        re.compile(r"^\./assets/js/market-base-pc-unified-shell-r95-v1\.js$"),
        re.compile(r"^\./assets/js/market-base-pc-unified-shell-v333-17\.js$"),
        re.compile(r"^\./assets/js/market-base-navigation-v333-17\.js$"),
        re.compile(r"^\./assets/css/market-base-navigation-v333-17\.css$"),
        re.compile(r"^\./assets/js/market-base-home-deferred-v333-17\.js$"),
    ...
    if failures:
        ...
        for failure in failures:
            print(f" - {failure}")
        return 1
    print("PASSED: V333.19 release integrity")
    return 0
