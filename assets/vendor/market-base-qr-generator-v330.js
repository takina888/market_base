/* MARKET BASE bundled QR generator. Source: qrcode-terminal vendor/QRCode, Apache-2.0. */
(function(global){
  'use strict';
  const modules={
...
module.exports = QRCode;
  ...
  const QRCode=requireModule('./index');
  const Levels=requireModule('./QRErrorCorrectLevel');
  function matrix(text, level){
    const qr=new QRCode(-1,Levels[String(level||'M').toUpperCase()] ?? Levels.M);
    qr.addData(String(text ?? ''));
    qr.make();
    return {count:qr.getModuleCount(), modules:qr.modules};
  }
  function render(canvas,text,options){
    options=options||{};
    const data=matrix(text,options.level||'M');
    const quiet=Math.max(2,Number(options.quiet||4));
    const requested=Math.max(160,Number(options.size||320));
    const units=data.count+quiet*2;
    const scale=Math.max(1,Math.floor(requested/units));
    ...
    ctx.fillStyle=options.background||'#fff'; ctx.fillRect(0,0,size,size);
    ctx.fillStyle=options.foreground||'#071f43';
    for(let row=0;row<data.count;row++) for(let col=0;col<data.count;col++){
      if(data.modules[row][col]) ctx.fillRect((col+quiet)*scale,(row+quiet)*scale,scale,scale);
    }
    return canvas;
  }
  global.MarketBaseQR=Object.freeze({matrix,render,license:'Apache-2.0'});
})(window);
