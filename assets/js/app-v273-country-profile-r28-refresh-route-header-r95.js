...
function switchView(view, opts={}){
  ...
  document.querySelector(`.bottom-tab[data-view="${targetView}"]`)?.classList.add('active');
...
const MARKET_BASE_APP_VERSION = 'MARKET BASE R113.14 LEARNING CHROME AND LOGISTICS TOP BAND 20260722';
...
async function purgeMarketBaseLocalCache(){
  const scopePath=marketBaseRootPath();
  if('caches' in window){
    const keys=await caches.keys();
    await Promise.all(keys.map(async key=>{
      const keyLooksLocal=/market[_-]?base|marketbase|mb-|mb_/i.test(String(key));
      if(keyLooksLocal) return caches.delete(key);
      try{
        const cache=await caches.open(key);
        const requests=await cache.keys();
        await Promise.all(requests.filter(req=>{
          try{ return new URL(req.url).pathname.startsWith(scopePath); }
...
async function checkMarketBaseVersion(){
  try{
    ...
    if(last===remote || runningTarget===remote) return;
...
async function refreshMarketBaseCache(event){
  ...
  try{
    ...
    sessionStorage.removeItem('market_base_auto_refresh_running');
    sessionStorage.removeItem('market_base_auto_refresh_target');
    localStorage.setItem('market_base_last_auto_refresh_version', MARKET_BASE_APP_VERSION);
    localStorage.setItem('market_base_last_cache_refresh', new Date().toISOString());
  }catch(err){
...
(function initPrimarySwipeNavigation(){
  const BLOCKED_SELECTOR=[
    ...
    '.bottom-nav','.tabs','.ranking-compare-tabs','.toolbar','.compare-selects','.compare-table',
