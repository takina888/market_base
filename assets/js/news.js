...
(function(global){
  ...
  const DEFAULT_CATEGORIES={
    ...
    food:'食品',
    food_machinery:'食品機械',
    trade_logistics:'貿易・物流',
    regulation_food_safety:'法規・食品安全'
  };
  ...
  function loadData(){
    if(!dataPromise){
      dataPromise=fetch(DATA_URL,{cache:'no-store'}).then(res=>{
        if(!res.ok) throw new Error(`news.json ${res.status}`);
        return res.json();
      }).then(normalizeDataset).then(data=>{ activeDataset=data; return data; }).catch(err=>{ dataPromise=null; throw err; });
    }
    return dataPromise;
  }
  function loadEntities(){
    if(!entityPromise){
      entityPromise=fetch(ENTITY_URL,{cache:'no-store'}).then(res=>{
        if(!res.ok) throw new Error(`country master ${res.status}`);
        return res.json();
      }).then(raw=>Array.isArray(raw?.entities)?raw.entities:[]).catch(err=>{entityPromise=null;throw err;});
    }
