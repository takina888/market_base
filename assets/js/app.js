// V84: flags are embedded in assets/flags/flag-svg-data.js. No split flag folders are required.
...
function flagMarkup(e, cls='flag-img'){
  ...
  const svg = window.MARKET_BASE_FLAG_SVG_DATA && window.MARKET_BASE_FLAG_SVG_DATA[key];
  if(svg){
    return `<span class="flag-asset ${safe(cls)}" role="img" aria-label="${name} flag">${svg}<span class="flag-emoji-fallback">${emoji}</span></span>`;
  }
  // V84 fallback: avoid references to old split flag folders. Emoji-only is safer than 404 image paths.
  return `<span class="flag-fallback-only ${safe(cls)}" role="img" aria-label="${name} flag">${emoji}</span>`;
}
