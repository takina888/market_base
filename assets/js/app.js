...
function retailCountrySummaryOverviewCards(){
  ...
  const head=`<section class="market-filter-card retail-light-load-card"><div class="market-filter-title"><strong>小売市場データベース</strong></div><p class="retail-note">本体では国別要約だけを表示しています。チェーン一覧・店舗数詳細・出典は別ページで開きます。</p><div class="retail-summary-metrics"><span><b>${Number(totalChains).toLocaleString()}</b>チェーン</span><span><b>${Number(totalCounts).toLocaleString()}</b>店舗数あり</span><span><b>${Number(list.length).toLocaleString()}</b>国・地域</span></div><div class="market-filter-actions"><a class="retail-summary-link" href="retail-market.html">小売市場DBを開く</a></div></section>`;
  ...
  const cards=shown.map(s=>{
    ...
    const link=s.detail_link_query || `retail-market.html?entity=${encodeURIComponent(s.entity_id||'')}`;
    return `<article class="market-data-card retail-country-summary-card"><div class="retail-card-head"><strong>${flagText(e)} ${safe(s.name_ja || nameOf(e) || s.entity_id)}</strong><span>${safe(s.entity_id||'')}</span></div><div class="retail-summary-metrics"><span><b>${Number(s.retail_chain_count||0).toLocaleString()}</b>チェーン</span><span><b>${Number(s.store_count_available_count||0).toLocaleString()}</b>店舗数あり</span><span><b>${Number(s.store_count_pending_count||0).toLocaleString()}</b>更新待ち</span></div><p class="retail-note">主な業態：${safe(s.primary_formats_label || '小売チェーン')}</p><p class="retail-note">企業本文：${Number(s.profile_text_count||0).toLocaleString()}件 / 四半期更新キュー：${Number(s.quarterly_update_queue_count||0).toLocaleString()}件${review?` / 範囲確認待ち：${Number(review).toLocaleString()}件`:''}</p><a class="retail-summary-link" href="${safe(link)}">詳細を見る</a></article>`;
  }).join('');
...
function retailEntitySummaryLink(entity, summary){
  ...
  const href=summary?.detail_link_query || `retail-market.html?entity=${encodeURIComponent(entityId)}`;
...
function retailEntitySummaryCard(entity){
  ...
  return `<article class="market-data-card retail-country-summary-card"><div class="retail-card-head"><strong>小売市場データベース</strong><span>要約表示</span></div><div class="retail-summary-metrics"><span><b>${Number(chains||0).toLocaleString()}</b>チェーン</span><span><b>${Number(available||0).toLocaleString()}</b>店舗数あり</span><span><b>${Number(pending||0).toLocaleString()}</b>更新待ち</span></div><p class="retail-note">主な業態：${safe(formats)}</p><p class="retail-note">企業本文：${Number(profiles||0).toLocaleString()}件 / 四半期更新キュー：${Number(queue||0).toLocaleString()}件${review?` / 範囲確認待ち：${Number(review).toLocaleString()}件`:''}</p>${retailEntitySummaryLink(entity, summary)}</article>`;
}
...
const SCHOOL_METRICS = {
  ...
  school_meals_total_schools:{label:'提供学校数ランキング', short:'提供学校数', unit:'校', priority:'main', emptyNote:'学校給食を提供している学校数データを取得中です。'},
  school_meals_coverage_pct:{label:'カバー率ランキング', short:'カバー率', unit:'%', priority:'main', emptyNote:'分母定義を確認中です。旧参考率は削除しました。'},
  school_meals_annual_serving_days:{label:'年間提供日数ランキング', short:'提供日数', unit:'日', priority:'main', emptyNote:'年間の給食提供日数データを取得中です。'}
};
const SCHOOL_RANKING_PRIORITY = ['school_meals_total_children','school_meals_total_schools','school_meals_coverage_pct','school_meals_annual_serving_days'];
...
function schoolSummaryCards(){
  ...
  <article class="market-data-card school-summary-card"><strong>ランキング</strong><span>提供児童数・学校数・カバー率・提供日数</span></article>`;
}
...
async function boot(){
  try{
    ...
    if(window.MARKET_BASE_EMBEDDED_DATA){
      ...
      // V227: fallback時も本体では小売詳細JSONを展開しない。
      retailData=null;
