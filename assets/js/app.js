// V170: splash failsafe from external JS as well.
(function(){
  function hide(){
    if(window.__mbHideSplash){ window.__mbHideSplash(); return; }
    var s=document.getElementById('appSplash');
    if(s){ s.classList.add('is-hidden'); setTimeout(function(){ if(s.parentNode) s.parentNode.removeChild(s); }, 500); }
  }
...
async function boot(){
  try{
    const [e,r,retail,retailPresence,retailProfiles,rice,riceRanks,schoolMeals,schoolRanks,schoolCk,priority4,wdiMeta,japanRel,japanRanks,jpRestaurantOverview,jpResidentsOverview]=await Promise.all([
...
// V170: short app-like splash screen with safe fallback.
(function(){
