// V119 market/retail UI tuned to match surrounding app tone; keeps V118 filters, V117 compare UI and CURRENT masters
...
const MARKET_DOMAINS = {
  ...
  rice:{title:'米データ', lead:'米生産・貿易・供給・用途・栄養を国別に整理するカテゴリです。', icon:'🍚', items:[['rice_production','米生産量'],['rice_import','米輸入量'],['rice_export','米輸出量'],['rice_food_supply','1人当たり米供給量']]},
...
function retailCategoryKey(record){
  ...
  if(raw.includes('convenience') || raw==='cvs') return 'convenience_store';
  if(raw.includes('hypermarket')) return 'hypermarket';
  if(raw.includes('warehouse') || raw.includes('club') || raw.includes('cash') || raw.includes('gms') || raw.includes('department')) return 'gms';
  if(raw.includes('supermarket') || raw.includes('grocery') || raw.includes('food') || raw.includes('market')) return 'supermarket';
  return 'other';
}
...
function retailAggregateCards(items, scopeLabel='収録チェーン集計'){
  ...
  return `<div class="retail-aggregate-block"><div class="retail-aggregate-head"><strong>${safe(scopeLabel)}</strong><span>詳細は下のチェーン一覧で確認</span></div><div class="retail-aggregate-grid">${cards}</div><p class="retail-aggregate-note">集計値は現在収録済みチェーンの店舗数合計です。市場全体推計ではなく、店舗数未確認のチェーンは件数だけ保持しています。</p></div>`;
}
...
function marketFilterOptions(){
  ...
  const entityIds=[...new Set(chains.map(x=>x.entity_id).filter(Boolean))].sort((a,b)=>nameOf(entities.find(e=>e.entity_id===a)||{}).localeCompare(nameOf(entities.find(e=>e.entity_id===b)||{})));
  const regions=[...new Set(entityIds.map(id=>entities.find(e=>e.entity_id===id)?.region).filter(Boolean))].sort();
  const cats=[...new Set(chains.map(x=>retailCategoryKey(x)).filter(Boolean))];
  return {entityIds, regions, cats};
}
...
function retailSummaryCards(){
  ...
  if(all.length){
    ...
    const more=filtered.length>shown.length ? `<article class="market-data-card retail-more-card"><strong>ほか ${(filtered.length-shown.length).toLocaleString()}件</strong><span>条件に合うチェーンをさらに表示できます。</span><button class="retail-more-button" type="button" onclick="marketVisibleLimit+=40; renderMarketScope();">もっと見る</button><em>詳細データ保持済み</em></article>` : '';
    return marketFilterControls() + retailAggregateCards(filtered,'絞り込み範囲の小売集計') + retailCompactSummary(filtered) + resultHead + shown.map(retailPresenceCard).join('') + more;
  }
...
const SCHOOL_METRICS = {
  school_meals_total_children:{label:'学校給食対象児童数', short:'対象児童', unit:'人'},
  primary_enrollment_children:{label:'小学校就学者数', short:'小学校', unit:'人'},
  pre_primary_enrollment_children:{label:'就学前教育在籍者数', short:'就学前', unit:'人'},
  school_meal_menu_gdqs_score:{label:'メニュー品質スコア', short:'品質', unit:'点'},
  school_meal_children_per_primary_enrollment_pct:{label:'学校給食対象児童数 / 小学校就学者数', short:'参考カバー率', unit:'%'}
};
...
function ricePriority4CardsForEntity(entity){
  ...
  return `<article class="market-data-card priority4-section-note"><strong>内部履歴から復元</strong><span>Priority4 READY候補 ${rows.length}件</span><em>FAOSTAT投入値と内部履歴候補を併記</em></article>` + rows.map(r=>priority4Card(r,'priority4-card rice-ready-card')).join('');
}
...
function schoolCardsFromData(entity){
  ...
  if(!rec) return '';
  return Object.keys(SCHOOL_METRICS).map(metric=>schoolDataCard(metric, rec?.[metric])).join('');
}
function schoolStatusSummary(){
  ...
  let any=0, complete=0, partial=0, notLoaded=0;
  records.forEach(r=>{
    const loaded=Object.keys(SCHOOL_METRICS).filter(k=>r[k]).length;
    if(loaded) any++;
    if(loaded===Object.keys(SCHOOL_METRICS).length) complete++;
    else if(loaded>0) partial++;
    else notLoaded++;
  });
  return {total:records.length, any, complete, partial, notLoaded};
...
function renderSchoolRanking(){
  ...
  if(!list) return;
  const metric=select?.value || 'school_meals_total_children';
...
function schoolMealCardsForEntity(entity){
  ...
  const p4 = rows.length ? `<article class="market-data-card priority4-section-note"><strong>内部履歴から復元</strong><span>Priority4 READY候補 ${rows.length}件</span><em>HOLD/RECHECKは非表示</em></article>` + rows.map(r=>priority4Card(r,'priority4-card school-ready-card')).join('') : '';
  if(dataCards || p4) return dataCards + p4;
...
function schoolSummaryCards(){
  ...
  const p4=priority4DomainSummary('school_meals');
  const status=s.any ? `値あり ${s.any}/${s.total}` : (p4.rows ? `READY候補 ${p4.rows}件` : '準備中');
  return `<article class="market-data-card school-summary-card"><strong>学校給食データ</strong><span>${safe(status)}</span><em>WFP/GCNF/UNESCO uploaded indicators</em></article>
  <article class="market-data-card school-summary-card"><strong>ランキング連動</strong><span>対象児童・就学者・品質</span><em>同じJSONから自動生成</em></article>
  <article class="market-data-card school-summary-card"><strong>内部履歴</strong><span>Priority4 READY ${safe(p4.rows)}件</span><em>HOLD/RECHECKは非表示</em></article>
  <article class="market-data-card school-summary-card"><strong>注意</strong><span>カバー率は参考比率</span><em>国別制度差に注意</em></article>`;
}
function riceSummaryCards(){
  ...
  return `<article class="market-data-card rice-summary-card"><strong>米データ v2</strong><span>${safe(statusText)}</span><em>${safe(meta.source_primary || 'FAOSTAT')} / ${safe(meta.update_policy || '月次更新想定')}</em></article>
    <article class="market-data-card rice-summary-card"><strong>対象指標</strong><span>生産・貿易・供給・用途・栄養</span><em>Food Balanceは供給ベース</em></article>
    ...
    <article class="market-data-card rice-summary-card"><strong>更新方法</strong><span>FAOSTAT正規化データを投入済み</span><em>Production / Trade / Food Balance</em></article>`;
}
...
function renderDomainPanel(domainKey, targetId){
  ...
  document.getElementById(targetId).innerHTML=`<article class="market-scope-hero"><div class="domain-icon-large">${d.icon}</div><div><h4>${safe(d.title)}</h4><p>${safe(d.lead)}</p><span class="market-state-chip">${stateChip}</span></div></article><div class="market-data-grid">${dataCards}</div><article class="market-note-card market-note-card-v119"><h4>国・地域別に確認</h4><p>小売データは国ページの集計値と、この市場ページの詳細一覧を組み合わせて確認します。店舗数が未確定のチェーンは推測せず、展開確認として表示します。</p><div class="quick-chip-row">${chips}</div></article>`;
}
...
function marketSummaryBlock(e){
  const blocks=Object.entries(MARKET_DOMAINS).map(([key,d])=>{
    ...
    const addNote = key==='market' && retailEntityHasData(e.entity_id) ? '<p class="source-line">小売は先にカテゴリ別集計を表示し、下に代表チェーンを出します。全チェーンは「もっと見る」で展開できます。ブランド出身国は補助情報です。</p>' : (key==='rice' ? '<p class="source-line">米データは公開用データを整理中です。未確定値や内部更新メモは表示しません。</p>' : (key==='japan' ? '<p class="source-line">日本関連は外務省・農水省・財務省等の確認済みデータを優先します。未取得値は未収録として表示します。</p>' : ''));
...
const COMPARE_SCHOOL_METRICS=['school_meals_total_children','primary_enrollment_children','pre_primary_enrollment_children','school_meal_menu_gdqs_score','school_meal_children_per_primary_enrollment_pct'];
...
async function boot(){
  try{
    const [e,r,retail,retailPresence,rice,riceRanks,schoolMeals,schoolRanks,priority4,wdiMeta,japanRel,japanRanks,jpRestaurantOverview,jpResidentsOverview]=await Promise.all([
