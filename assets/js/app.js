...
function retailInfoLabel(record){
  ...
  if(raw.includes('確認')) return '展開情報';
  return '展開情報';
}
...
function retailCountrySummaryOverviewCards(){
  ...
  const cards=shown.map(s=>{
    ...
    return `<article class="market-data-card retail-country-summary-card"><div class="retail-card-head"><strong>${flagText(e)} ${safe(s.name_ja || nameOf(e) || s.entity_id)}</strong><span>${safe(s.entity_id||'')}</span></div><div class="retail-summary-metrics"><span><b>${Number(s.retail_chain_count||0).toLocaleString()}</b>チェーン</span><span><b>${Number(s.store_count_available_count||0).toLocaleString()}</b>店舗数あり</span><span><b>${Number(s.store_count_pending_count||0).toLocaleString()}</b>更新待ち</span></div><p class="retail-note">主な業態：${safe(s.primary_formats_label || '小売チェーン')}</p><p class="retail-note">企業本文：${Number(s.profile_text_count||0).toLocaleString()}件 / 四半期更新キュー：${Number(s.quarterly_update_queue_count||0).toLocaleString()}件${review?` / 範囲調整中：${Number(review).toLocaleString()}件`:''}</p><a class="retail-summary-link" href="${safe(link)}">詳細を見る</a></article>`;
  }).join('');
...
function priority4ValueText(row){
  const v=row?.value;
  if(v===null || v===undefined || v==='') return '掲載候補';
  const n=Number(v);
  const unit=row.unit || '';
  if(Number.isFinite(n)){
...
const MARKET_BASE_APP_VERSION = 'V243_CVS_PASS53_RC_V5_DISPLAY_REPLACE_20260707';
