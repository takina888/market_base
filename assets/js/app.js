...
let cvsVendorCountrySummaryData=null;
let cvsVendorCompaniesData=null;
let cvsVendorFactoriesData=null;
let cvsVendorCandidatesData=null;
let cvsVendorTasksData=null;
let riceData=null;
let riceRankings={};
let schoolMealsData=null;
let schoolMealRankings={};
let schoolCentralKitchenData=null;
...
function renderRecentEntities(){
  ...
  const recent=ids.map(id=>entities.find(e=>e.entity_id===id)).filter(Boolean);
  if(!recent.length){
    el.innerHTML=`<article class="recent-empty-card"><strong>${safe(ht('noRecent'))}</strong><span>${safe(ht('recentHelp'))}</span></article>`;
    return;
  }
...
function applyI18n(){
  ...
  if(typeof applySchoolI18nStatic === 'function') applySchoolI18nStatic();
  if(typeof applyRetailHomeI18nStatic === 'function') applyRetailHomeI18nStatic();
  if(typeof applyHomeI18nStatic === 'function') applyHomeI18nStatic();
  if(typeof applyCvsI18nStatic === 'function') applyCvsI18nStatic();
}
...
function retailRecordCard(record){
  ...
  const link=sourceLink(retailSourceUrl(record), rui('source'));
  return `<article class="market-data-card retail-record-card ${retailInfoClass(record)}"><div class="retail-card-head"><strong>${safe(retailDisplayName(record.chain_name || record.retail_format_display_ja || rui('retailData')))}</strong><span>${safe(retailChannelLabel(record.retail_format_display_ja || record.channel_type || record.retail_format_id || '小売'))}</span></div>${retailStoreCountLine(record)}${origin?`<span class="retail-origin-chip">${origin}</span>`:''}<p class="retail-note">${safe(retailJapaneseNote(record))}</p><div class="retail-source-line">${link}</div></article>`;
}
function retailPresenceCard(chain){
  ...
  const link=sourceLink(retailSourceUrl(chain), rui('source'));
  const note=retailJapaneseNote(chain);
  return `<article class="market-data-card retail-presence-card retail-compact-card ${retailInfoClass(chain)}"><div class="retail-card-top"><div><strong>${safe(retailDisplayName(chain.chain_name || rui('retailChain')))}</strong>${countryLine}</div><span class="retail-category-pill">${safe(retailChannelLabel(chain.channel_type || '小売'))}</span></div><div class="retail-card-facts"><span class="retail-store-pill">${safe(countText)}</span><span class="retail-info-chip">${safe(date)}・${safe(info)}</span>${origin?`<span class="retail-origin-chip">${origin}</span>`:''}</div>${note?`<p class="retail-note retail-note-compact">${safe(note)}</p>`:''}<div class="retail-source-line">${link}</div></article>`;
}
...
function linkedStoreCountLine(profile){
  ...
  if(!counts.length) return `<span class="retail-store-pill profile-store-unlinked">${safe(rui('storeCountUnlinked'))}</span>`;
...
function retailProfileCard(profile, compact=false){
  ...
  return `<article class="market-data-card retail-profile-card ${compact?'retail-profile-card-compact':''}"><div class="retail-card-top"><div><strong>${safe(retailDisplayName(profile.display_name_ja || profile.original_name || rui('retailStore')))}</strong>${subtitle}${country}</div><span class="retail-category-pill">${safe(profile.category_ja || rui('retail'))}</span></div><div class="retail-card-facts">${linkedStoreCountLine(profile)}<span class="retail-info-chip">${safe(linkStatus)}</span></div>${excerpt}<div class="retail-source-line"><button class="retail-more-button" type="button" onclick="openRetailProfile('${safe(profile.profile_id)}')">${safe(rui('detailProfile'))}</button></div></article>`;
}
function retailProfileSummaryBlock(){
  ...
  return `<section class="retail-profile-section"><div class="retail-profile-section-head"><strong>${safe(rui('retailProfiles'))}</strong><span>${profiles.length.toLocaleString()}${safe(rui('countSuffix'))}${hasFilter?' / '+safe(rui('matched')):''}・${safe(rui('withText'))} ${textProfiles.toLocaleString()}${safe(rui('countSuffix'))}</span></div><div class="market-data-grid compact">${visible.map(p=>retailProfileCard(p,true)).join('')}</div>${profiles.length>visible.length?`<div class="market-filter-actions"><button onclick="marketVisibleLimit+=24; renderMarketScope();">${safe(rui('showMoreProfiles'))}</button></div>`:''}</section>`;
}
function retailProfileCardsForEntity(e){
  ...
  return `<div class="retail-profile-country-head"><strong>${safe(rui('retailProfiles'))}</strong><span>${safe(rui('profileCountryHeadLead'))}</span></div>${body}${more}`;
}
...
function openRetailProfile(profileId){
  ...
  document.getElementById('detailContent').innerHTML=`<div class="detail-header retail-profile-detail-header"><div class="detail-flag retail-profile-initial">${safe((retailDisplayName(profile.display_name_ja||profile.original_name)||'小').slice(0,2))}</div><div class="detail-title-block"><h2>${safe(retailDisplayName(profile.display_name_ja || profile.original_name || rui('retailProfiles')))}</h2>${subtitle?`<p class="country-sub">${safe(subtitle)}</p>`:''}<div class="detail-mini-tabs"><span>${safe(rui('profile'))}</span><span>${safe(rui('profileBody'))}</span><span>${safe(rui('storeCountLink'))}</span></div></div></div><section class="detail-section retail-profile-detail"><h3>${e?`${flagText(e)} ${safe(nameOf(e))}`:'小売店'}｜${safe(profile.category_ja || rui('retail'))}</h3>${profileInfoRows(profile)}<div class="retail-card-facts retail-profile-linked-line">${linkedStoreCountLine(profile)}${linkedSource}</div><article class="market-note-card retail-profile-body"><h4>${safe(rui('profileBody'))}</h4><p>${body}</p></article><p class="source-line">${safe(rui('storeCountNote'))}</p></section>`;
...
function marketFilterControls(){
  ...
  return `<section class="market-filter-card market-filter-card-v119"><div class="market-filter-title"><strong>${safe(rui('marketFilterTitle'))}</strong></div><label class="market-search-wrap"><span>⌕</span><input class="market-search-input" value="${safe(marketFilters.q||'')}" placeholder="${safe(rui('marketSearchPlaceholder'))}" oninput="updateMarketFilter('q', this.value)"></label><div class="market-filter-grid"><select onchange="updateMarketFilter('region', this.value)">${regionOpts}</select><select onchange="updateMarketFilter('entity', this.value)">${entityOpts}</select><select onchange="updateMarketFilter('category', this.value)">${catOpts}</select><select onchange="updateMarketFilter('info', this.value)"><option value="all">${safe(rui('allSourceTypes'))}</option><option value="official" ${marketFilters.info==='official'?'selected':''}>${safe(rui('officialSemi'))}</option><option value="reference" ${marketFilters.info==='reference'?'selected':''}>${safe(rui('referenceValue'))}</option><option value="presence" ${marketFilters.info==='presence'?'selected':''}>${safe(rui('presenceConfirmed'))}</option></select><select onchange="updateMarketFilter('count', this.value)"><option value="all">${safe(rui('allStoreCounts'))}</option><option value="with" ${marketFilters.count==='with'?'selected':''}>${safe(rui('withStoreCount'))}</option><option value="without" ${marketFilters.count==='without'?'selected':''}>${safe(rui('withoutStoreCount'))}</option></select></div>${activeHtml}<div class="market-filter-actions"><button onclick="resetMarketFilters()">${safe(rui('clearFilters'))}</button><button onclick="marketVisibleLimit+=48; renderMarketScope();">${safe(rui('showMore'))}</button></div></section>`;
}
...
function retailSummaryCards(){
  ...
  if(all.length){
    ...
    return marketFilterControls() + retailAggregateCards(filtered,rui('filteredRetailSummary')) + retailCompactSummary(filtered) + retailProfileSummaryBlock() + resultHead + shown.map(retailPresenceCard).join('') + more;
  }
  ...
  return retailAggregateCards(records,rui('overallRetailSummary')) + retailCompactSummary(records) + retailLimitedCards(records, retailRecordCard, 20, 'retail:summary');
}
function retailEntityCards(entity){
  const chains=retailChainsForEntity(entity.entity_id);
  if(chains.length) return retailAggregateCards(chains,`${safe(nameOf(entity))}${safe(rui('retailSummaryOf'))}`) + retailCompactSummary(chains) + retailLimitedCards(chains, retailPresenceCard, 5, `retail:entity:${entity.entity_id}`);
  const records=retailRecordsForEntity(entity.entity_id);
  if(!records.length) return MARKET_DOMAINS.market.items.map(x=>notCollectedCard(x[0],x[1])).join('');
  return retailAggregateCards(records,`${safe(nameOf(entity))}${safe(rui('retailSummaryOf'))}`) + retailCompactSummary(records) + retailLimitedCards(records, retailRecordCard, 5, `retail:entity:${entity.entity_id}`);
}
...
function renderSchoolPanel(){
  ...
  panel.innerHTML=`<section class="retail-profile-section school-profile-section">
    ...
    ${rows.length>visible.length?`<div class="market-filter-actions"><button onclick="schoolVisibleLimit+=24; renderSchoolPanel();">${safe(st('showMore'))}</button></div>`:''}
  </section>`;
}
...
const CVS_I18N={
  ja:{dbTitle:'国別コンビニ食品ベンダー工場データベース', dbHomeLead:'コンビニ向け食品を製造する会社、工場・拠点、顧客チェーン、確認状況を国ごとに整理しています。', more:'詳しく見る →', company:'会社', factory:'工場・拠点', candidate:'追加調査候補', countries:'国・地域', core:'Core情報', growth:'追加検索で育てるDB', detail:'詳細を見る', next:'次に調べる', factories:'工場・拠点', companies:'会社', candidates:'候補', customer:'顧客チェーン', category:'製造カテゴリ', status:'確認状況', source:'出典', noData:'CVS食品ベンダー工場データは未収録です。', hiddenNote:'商品名・商品ラベル証拠は通常表示から外しています。'},
  en:{dbTitle:'Country CVS food vendor factory database', dbHomeLead:'Organizes companies, factories/sites, customer chains, and verification status for convenience-store food production by country.', more:'View details →', company:'Companies', factory:'Factories/sites', candidate:'Research candidates', countries:'countries/areas', core:'Core data', growth:'Growth database', detail:'View details', next:'Next research', factories:'Factories/sites', companies:'Companies', candidates:'Candidates', customer:'Customer chains', category:'Manufacturing category', status:'Verification status', source:'Sources', noData:'No CVS food vendor factory data recorded yet.', hiddenNote:'Product names and product-label evidence are not shown in the normal UI.'},
  zh_tw:{dbTitle:'各國便利商店食品供應商工廠資料庫', dbHomeLead:'依國家整理便利商店食品製造公司、工廠／據點、客戶通路與確認狀況。', more:'查看詳情 →', company:'公司', factory:'工廠／據點', candidate:'追加調查候選', countries:'國家／地區', core:'Core資訊', growth:'可追加調查的成長型資料庫', detail:'查看詳情', next:'下一步調查', factories:'工廠／據點', companies:'公司', candidates:'候選', customer:'客戶通路', category:'製造類別', status:'確認狀況', source:'來源', noData:'尚未收錄便利商店食品供應商工廠資料。', hiddenNote:'商品名稱與商品標籤證據不在一般畫面顯示。'},
  zh_cn:{dbTitle:'各国便利店食品供应商工厂数据库', dbHomeLead:'按国家整理便利店食品制造公司、工厂／据点、客户渠道与确认状态。', more:'查看详情 →', company:'公司', factory:'工厂／据点', candidate:'追加调查候选', countries:'国家／地区', core:'Core信息', growth:'可追加调查的成长型数据库', detail:'查看详情', next:'下一步调查', factories:'工厂／据点', companies:'公司', candidates:'候选', customer:'客户渠道', category:'制造类别', status:'确认状态', source:'来源', noData:'尚未收录便利店食品供应商工厂数据。', hiddenNote:'商品名称与商品标签证据不在普通界面显示。'}
};
function cvst(key){ return (CVS_I18N[lang] && CVS_I18N[lang][key]) || CVS_I18N.ja[key] || key; }
function applyCvsI18nStatic(){
  document.querySelectorAll('[data-cvs-i18n]').forEach(el=>{ el.textContent=cvst(el.dataset.cvsI18n); });
  document.querySelectorAll('[data-cvs-i18n-aria]').forEach(el=>{ el.setAttribute('aria-label', cvst(el.dataset.cvsI18nAria)); });
}
function cvsCountries(){ return cvsVendorCountrySummaryData?.countries || []; }
function cvsCompanies(){ return cvsVendorCompaniesData?.companies || []; }
function cvsFactories(){ return cvsVendorFactoriesData?.factories || []; }
function cvsCandidates(){ return cvsVendorCandidatesData?.candidates || []; }
function cvsTasks(){ return cvsVendorTasksData?.tasks || []; }
function cvsSummary(){
  const meta=cvsVendorCountrySummaryData?.meta || {};
  const c=meta.counts || {};
  return {countries:c.country_summary_records||cvsCountries().length, companies:c.core_companies||cvsCompanies().length, factories:c.core_factories||cvsFactories().length, candidates:c.research_candidates||cvsCandidates().length};
}
function cvsHomeBlock(){
  const s=cvsSummary();
  if(!s.countries) return `<article class="market-note-card cvs-vendor-home"><h4>${safe(cvst('dbTitle'))}</h4><p>${safe(cvst('noData'))}</p></article>`;
  const top=cvsCountries().slice().sort((a,b)=>(b.core_factory_count||0)-(a.core_factory_count||0)).slice(0,6);
  const countryCards=top.map(r=>`<button class="cvs-country-mini" onclick="openCvsCountry('${safe(r.country_area_ja)}')"><strong>${safe(r.country_area_ja)}</strong><span>${safe(cvst('company'))} ${safe(r.core_company_count||0)} / ${safe(cvst('factory'))} ${safe(r.core_factory_count||0)}</span><em>${safe(cvst('candidate'))} ${safe(r.research_candidate_count||0)}</em></button>`).join('');
  return `<section class="retail-profile-section cvs-vendor-section"><div class="retail-profile-section-head"><strong>${safe(cvst('dbTitle'))}</strong><span>${safe(cvst('growth'))}</span></div>
    <div class="cvs-stat-grid"><article><strong>${safe(s.companies)}</strong><span>${safe(cvst('companies'))}</span></article><article><strong>${safe(s.factories)}</strong><span>${safe(cvst('factories'))}</span></article><article><strong>${safe(s.candidates)}</strong><span>${safe(cvst('candidates'))}</span></article><article><strong>${safe(s.countries)}</strong><span>${safe(cvst('countries'))}</span></article></div>
    <p class="section-lead">${safe(cvst('dbHomeLead'))}</p><p class="section-lead cvs-hidden-note">${safe(cvst('hiddenNote'))}</p>
    <div class="cvs-country-grid">${countryCards}</div>
  </section>`;
}
function openCvsCountry(country){
  const rec=cvsCountries().find(x=>x.country_area_ja===country);
  if(!rec) return;
  const comps=cvsCompanies().filter(x=>x.country_area_ja===country).slice(0,18);
  const facs=cvsFactories().filter(x=>x.country_area_ja===country).slice(0,18);
  const cands=cvsCandidates().filter(x=>x.country_area_ja===country).slice(0,10);
  const tasks=cvsTasks().filter(x=>x.country_area_ja===country).slice(0,3);
  const compHtml=comps.length?comps.map(x=>`<article><h4>${safe(x.company_name||'')}</h4><div class="retail-card-facts">${x.main_customers?`<span class="retail-info-chip">${safe(cvst('customer'))}: ${safe(x.main_customers).slice(0,120)}</span>`:''}${x.confirmation_status?`<span class="retail-info-chip">${safe(cvst('status'))}: ${safe(x.confirmation_status).slice(0,80)}</span>`:''}</div><p>${safe(x.manufacturing_category||x.handled_products_or_functions||'')}</p></article>`).join(''):`<p>${safe(cvst('noData'))}</p>`;
  const facHtml=facs.length?facs.map(x=>`<article><h4>${safe(x.factory_name||x.company_name||'')}</h4><p><strong>${safe(x.company_name||'')}</strong>${x.factory_location?` / ${safe(x.factory_location)}`:''}</p><div class="retail-card-facts">${x.manufacturing_category?`<span class="retail-info-chip">${safe(cvst('category'))}: ${safe(x.manufacturing_category).slice(0,100)}</span>`:''}${x.capacity_or_scale?`<span class="retail-info-chip">${safe(x.capacity_or_scale).slice(0,90)}</span>`:''}</div></article>`).join(''):`<p>${safe(cvst('noData'))}</p>`;
  const candHtml=cands.length?cands.map(x=>`<li><strong>${safe(x.candidate_name||x.reason_for_watch||'')}</strong><span>${safe(x.reason_for_watch||x.confirmation_status||'')}</span></li>`).join(''):`<li>${safe(cvst('noData'))}</li>`;
  const taskHtml=tasks.length?tasks.flatMap(t=>t.suggested_queries||[]).slice(0,8).map(q=>`<li>${safe(q)}</li>`).join(''):(rec.next_research_focus||[]).map(q=>`<li>${safe(q)}</li>`).join('');
  document.getElementById('detailContent').innerHTML=`<div class="detail-header retail-profile-detail-header"><div class="detail-flag retail-profile-initial">CVS</div><div class="detail-title-block"><h2>${safe(rec.country_area_ja)}｜${safe(cvst('dbTitle'))}</h2><p class="country-sub">${safe(cvst('growth'))}</p><div class="detail-mini-tabs"><span>${safe(cvst('companies'))}</span><span>${safe(cvst('factories'))}</span><span>${safe(cvst('next'))}</span></div></div></div>
  <section class="detail-section retail-profile-detail cvs-detail"><h3>${safe(rec.country_area_ja)}</h3><div class="retail-profile-info-grid"><div><span>${safe(cvst('companies'))}</span><strong>${safe(rec.core_company_count||0)}</strong></div><div><span>${safe(cvst('factories'))}</span><strong>${safe(rec.core_factory_count||0)}</strong></div><div><span>${safe(cvst('candidates'))}</span><strong>${safe(rec.research_candidate_count||0)}</strong></div></div>
  <article class="market-note-card retail-profile-body"><h4>${safe(cvst('companies'))}</h4><div class="cvs-detail-list">${compHtml}</div></article>
  <article class="market-note-card retail-profile-body"><h4>${safe(cvst('factories'))}</h4><div class="cvs-detail-list">${facHtml}</div></article>
  <article class="market-note-card retail-profile-body"><h4>${safe(cvst('candidate'))}</h4><ul>${candHtml}</ul></article>
  <article class="market-note-card retail-profile-body"><h4>${safe(cvst('next'))}</h4><ul>${taskHtml}</ul></article>
  <p class="source-line">${safe(cvst('hiddenNote'))}</p></section>`;
  switchView('detail');
}
...
function renderDomainPanel(domainKey, targetId){
  ...
  const dataCards = domainKey==='market' ? (retailSummaryCards() + cvsHomeBlock()) : (domainKey==='rice' ? riceSummaryCards() : (domainKey==='japan' ? japanRelatedSummaryCards() : d.items.map(x=>notCollectedCard(x[0],x[1])).join('')));
...
function companySearchResults(qRaw){
  ...
  allRetailProfiles().forEach(p=>{
    ...
    results.push({kind:'profile', id:p.profile_id, entity_id:p.entity_id, country:e?nameOf(e):p.country_area_ja, flag:e?flagText(e):'', name:p.display_name_ja || p.original_name || '小売店', sub:retailDisplayName(p.sub_name_ja || p.original_name || ''), category:p.category_ja || '小売', storeText:companyStoreTextFromProfile(p), hasText:!!p.profile_text, score:hasTextQuery?((String(p.display_name_ja||'').toLowerCase()===q||String(p.sub_name_ja||'').toLowerCase()===q)?0:1):1});
  });
  allRetailPresenceChains().forEach(c=>{
    ...
    if(seen.has(key)) return; seen.add(key);
    results.push({kind:p?'profile':'chain', id:p?.profile_id || '', entity_id:c.entity_id, country:e?nameOf(e):c.country_area, flag:e?flagText(e):'', name:retailDisplayName(c.chain_name || c.source_name || rui('retailChain')), sub:retailDisplayName(p?.display_name_ja || ''), category:retailCategoryLabel(retailCategoryKey(c)), storeText:companyStoreTextFromChain(c), hasText:!!p?.profile_text, score:hasTextQuery?(String(c.chain_name||'').toLowerCase()===q?0:2):2});
  });
...
const HOME_I18N={
  ja:{
    recentBadge:'RECENT AREAS', recentTitle:'最近見た国・地域', findCountries:'国・地域一覧から探す', clearHistory:'履歴をクリア', marketEntryTitle:'国・地域から市場へ進む',
    marketRetail:'市場・小売', riceData:'米データ', schoolMeals:'学校給食', japanRelated:'日本関連', rankings:'ランキング', compare:'比較',
    noRecent:'まだ履歴はありません', recentHelp:'国・地域の詳細を開くと、ここに最大6件まで表示されます。',
    releaseBadge:'196の国・地域', targetSearchLabel:'国名・地域・キーワードで検索', howToFind:'探し方', findByCountry:'国から探す', findByCountryLead:'196の国・地域から選ぶ',
    findByRanking:'ランキングから探す', findByRankingLead:'指標のランキングを見る', findByRegion:'地域から探す', findByRegionLead:'地域ごとに国を一覧で見る',
    recentCountries:'最近見た国', viewAll:'すべて見る ›', eastAsia:'東アジア', southeastAsia:'東南アジア', europe:'ヨーロッパ', northAmerica:'北米', latinAmerica:'中南米',
    middleEast:'中東', africa:'アフリカ', oceania:'オセアニア', findByCompany:'企業名から探す', allRegions:'地域すべて', allCountriesAreas:'国・地域すべて',
    popularRankings:'人気ランキング', population:'人口', riceProduction:'米生産量', overseasJapanese:'在留邦人数', viewRankings:'ランキングを見る',
    viewMarketInfo:'市場情報を見る', viewSourcesInCountry:'国詳細で出典を見る', countriesAreasShort:'国・地域', basicMetrics:'基本指標', dataLabel:'データ', updated:'更新済み',
    home:'ホーム', countryList:'国一覧', companySearchPlaceholder:'例：CU / Carrefour / AEON',
    allSubregions:'小地域すべて', allIncomeGroups:'所得分類すべて', marketShort:'市場', preparing:'整備中', riceRankings:'米ランキング', japanShort:'日本', japanRelatedRankings:'日本関連ランキング',
    riceArea:'米収穫面積', riceYield:'米単収', riceImportQty:'米輸入量', riceExportQty:'米輸出量', riceImportValue:'米輸入額', riceExportValue:'米輸出額', riceDomesticSupply:'国内供給量', riceFoodUse:'食品用途量', riceFeedUse:'飼料用途量', riceProcessingUse:'加工用途量', riceSeedUse:'種子用途量', riceLosses:'損失量', riceStockVariation:'在庫変動量', riceOtherUses:'その他用途量', riceFoodSupplyPerCapita:'1人当たり米供給量', riceKcalSupply:'1人当たりカロリー供給', riceProteinSupply:'1人当たりタンパク質供給', riceFatSupply:'1人当たり脂質供給',
    japanRelationSummary:'日本との関係まとめ', overseasJapaneseRegionTotal:'在留邦人数 地域合計', longTermResidents:'長期滞在者数', permanentResidents:'永住者数', overseasJapaneseYoy:'在留邦人数 前年比', japaneseRestaurants:'日本食レストラン数', japaneseRestaurantsRegionTotal:'日本食レストラン数 地域合計', japaneseRetailFoodservice:'日系小売・外食の存在'
  },
  en:{
    recentBadge:'RECENT AREAS', recentTitle:'Recently viewed countries/areas', findCountries:'Find from country/area list', clearHistory:'Clear history', marketEntryTitle:'Start from a country/area',
    marketRetail:'Market / Retail', riceData:'Rice data', schoolMeals:'School meals', japanRelated:'Japan-related', rankings:'Rankings', compare:'Compare',
    noRecent:'No history yet', recentHelp:'Open country/area details to show up to 6 recent items here.',
    releaseBadge:'196 countries/areas', targetSearchLabel:'Search by country, area, or keyword', howToFind:'How to find', findByCountry:'Find by country', findByCountryLead:'Choose from 196 countries/areas',
    findByRanking:'Find by ranking', findByRankingLead:'View indicator rankings', findByRegion:'Find by region', findByRegionLead:'Browse countries by region',
    recentCountries:'Recently viewed countries', viewAll:'View all ›', eastAsia:'East Asia', southeastAsia:'Southeast Asia', europe:'Europe', northAmerica:'North America', latinAmerica:'Latin America',
    middleEast:'Middle East', africa:'Africa', oceania:'Oceania', findByCompany:'Find by company name', allRegions:'All regions', allCountriesAreas:'All countries/areas',
    popularRankings:'Popular rankings', population:'Population', riceProduction:'Rice production', overseasJapanese:'Japanese residents abroad', viewRankings:'View rankings',
    viewMarketInfo:'View market information', viewSourcesInCountry:'View sources in country details', countriesAreasShort:'Countries/areas', basicMetrics:'Basic metrics', dataLabel:'Data', updated:'Updated',
    home:'Home', countryList:'Country list', companySearchPlaceholder:'Example: CU / Carrefour / AEON',
    allSubregions:'All subregions', allIncomeGroups:'All income groups', marketShort:'Market', preparing:'Preparing', riceRankings:'Rice rankings', japanShort:'Japan', japanRelatedRankings:'Japan-related rankings',
    riceArea:'Rice harvested area', riceYield:'Rice yield', riceImportQty:'Rice import quantity', riceExportQty:'Rice export quantity', riceImportValue:'Rice import value', riceExportValue:'Rice export value', riceDomesticSupply:'Domestic supply', riceFoodUse:'Food use', riceFeedUse:'Feed use', riceProcessingUse:'Processing use', riceSeedUse:'Seed use', riceLosses:'Losses', riceStockVariation:'Stock variation', riceOtherUses:'Other uses', riceFoodSupplyPerCapita:'Rice supply per capita', riceKcalSupply:'Calories from rice per capita', riceProteinSupply:'Protein from rice per capita', riceFatSupply:'Fat from rice per capita',
    japanRelationSummary:'Japan relationship summary', overseasJapaneseRegionTotal:'Japanese residents abroad by region', longTermResidents:'Long-term residents', permanentResidents:'Permanent residents', overseasJapaneseYoy:'Japanese residents abroad YoY', japaneseRestaurants:'Japanese restaurants', japaneseRestaurantsRegionTotal:'Japanese restaurants by region', japaneseRetailFoodservice:'Japanese retail / foodservice presence'
  },
  zh_tw:{
    recentBadge:'最近瀏覽', recentTitle:'最近看過的國家／地區', findCountries:'從國家／地區列表尋找', clearHistory:'清除紀錄', marketEntryTitle:'從國家／地區進入市場資訊',
    marketRetail:'市場／零售', riceData:'稻米資料', schoolMeals:'學校供餐', japanRelated:'日本相關', rankings:'排名', compare:'比較',
    noRecent:'尚無瀏覽紀錄', recentHelp:'開啟國家／地區詳細頁後，這裡最多會顯示6筆最近項目。',
    releaseBadge:'196個國家／地區', targetSearchLabel:'以國家、地區或關鍵字搜尋', howToFind:'尋找方式', findByCountry:'從國家尋找', findByCountryLead:'從196個國家／地區選擇',
    findByRanking:'從排名尋找', findByRankingLead:'查看指標排名', findByRegion:'從地區尋找', findByRegionLead:'依地區瀏覽國家',
    recentCountries:'最近看過的國家', viewAll:'查看全部 ›', eastAsia:'東亞', southeastAsia:'東南亞', europe:'歐洲', northAmerica:'北美', latinAmerica:'中南美',
    middleEast:'中東', africa:'非洲', oceania:'大洋洲', findByCompany:'依公司名稱尋找', allRegions:'所有地區', allCountriesAreas:'所有國家／地區',
    popularRankings:'熱門排名', population:'人口', riceProduction:'稻米產量', overseasJapanese:'海外日僑人數', viewRankings:'查看排名',
    viewMarketInfo:'查看市場資訊', viewSourcesInCountry:'在國家詳細頁查看來源', countriesAreasShort:'國家／地區', basicMetrics:'基本指標', dataLabel:'資料', updated:'已更新',
    home:'首頁', countryList:'國家列表', companySearchPlaceholder:'例：CU / Carrefour / AEON',
    allSubregions:'所有小地區', allIncomeGroups:'所有所得分類', marketShort:'市場', preparing:'整理中', riceRankings:'稻米排名', japanShort:'日本', japanRelatedRankings:'日本相關排名',
    riceArea:'稻米收穫面積', riceYield:'稻米單產', riceImportQty:'稻米進口量', riceExportQty:'稻米出口量', riceImportValue:'稻米進口額', riceExportValue:'稻米出口額', riceDomesticSupply:'國內供給量', riceFoodUse:'食用量', riceFeedUse:'飼料用途量', riceProcessingUse:'加工用途量', riceSeedUse:'種子用途量', riceLosses:'損耗量', riceStockVariation:'庫存變動量', riceOtherUses:'其他用途量', riceFoodSupplyPerCapita:'每人稻米供給量', riceKcalSupply:'每人每日稻米熱量供給', riceProteinSupply:'每人每日稻米蛋白質供給', riceFatSupply:'每人每日稻米脂質供給',
    japanRelationSummary:'與日本的關係摘要', overseasJapaneseRegionTotal:'海外日僑人數 地區合計', longTermResidents:'長期居留者數', permanentResidents:'永久居留者數', overseasJapaneseYoy:'海外日僑人數年增率', japaneseRestaurants:'日本料理餐廳數', japaneseRestaurantsRegionTotal:'日本料理餐廳數 地區合計', japaneseRetailFoodservice:'日系零售／餐飲存在'
  },
  zh_cn:{
    recentBadge:'最近浏览', recentTitle:'最近看过的国家／地区', findCountries:'从国家／地区列表查找', clearHistory:'清除记录', marketEntryTitle:'从国家／地区进入市场信息',
    marketRetail:'市场／零售', riceData:'稻米数据', schoolMeals:'学校供餐', japanRelated:'日本相关', rankings:'排名', compare:'比较',
    noRecent:'暂无浏览记录', recentHelp:'打开国家／地区详情页后，这里最多会显示6条最近项目。',
    releaseBadge:'196个国家／地区', targetSearchLabel:'按国家、地区或关键词搜索', howToFind:'查找方式', findByCountry:'按国家查找', findByCountryLead:'从196个国家／地区中选择',
    findByRanking:'按排名查找', findByRankingLead:'查看指标排名', findByRegion:'按地区查找', findByRegionLead:'按地区浏览国家',
    recentCountries:'最近看过的国家', viewAll:'查看全部 ›', eastAsia:'东亚', southeastAsia:'东南亚', europe:'欧洲', northAmerica:'北美', latinAmerica:'拉丁美洲',
    middleEast:'中东', africa:'非洲', oceania:'大洋洲', findByCompany:'按公司名称查找', allRegions:'所有地区', allCountriesAreas:'所有国家／地区',
    popularRankings:'热门排名', population:'人口', riceProduction:'稻米产量', overseasJapanese:'海外日本人数', viewRankings:'查看排名',
    viewMarketInfo:'查看市场信息', viewSourcesInCountry:'在国家详情页查看来源', countriesAreasShort:'国家／地区', basicMetrics:'基本指标', dataLabel:'数据', updated:'已更新',
    home:'首页', countryList:'国家列表', companySearchPlaceholder:'例：CU / Carrefour / AEON',
    allSubregions:'所有小地区', allIncomeGroups:'所有收入分类', marketShort:'市场', preparing:'整理中', riceRankings:'稻米排名', japanShort:'日本', japanRelatedRankings:'日本相关排名',
    riceArea:'稻米收获面积', riceYield:'稻米单产', riceImportQty:'稻米进口量', riceExportQty:'稻米出口量', riceImportValue:'稻米进口额', riceExportValue:'稻米出口额', riceDomesticSupply:'国内供给量', riceFoodUse:'食用量', riceFeedUse:'饲料用途量', riceProcessingUse:'加工用途量', riceSeedUse:'种子用途量', riceLosses:'损耗量', riceStockVariation:'库存变动量', riceOtherUses:'其他用途量', riceFoodSupplyPerCapita:'人均稻米供给量', riceKcalSupply:'人均每日稻米热量供给', riceProteinSupply:'人均每日稻米蛋白质供给', riceFatSupply:'人均每日稻米脂肪供给',
    japanRelationSummary:'与日本的关系摘要', overseasJapaneseRegionTotal:'海外日本人数 地区合计', longTermResidents:'长期居留者数', permanentResidents:'永久居留者数', overseasJapaneseYoy:'海外日本人数同比', japaneseRestaurants:'日本料理餐厅数', japaneseRestaurantsRegionTotal:'日本料理餐厅数 地区合计', japaneseRetailFoodservice:'日系零售／餐饮存在'
  }
};
function ht(key){ return (HOME_I18N[lang] && HOME_I18N[lang][key]) || HOME_I18N.ja[key] || key; }
function applyHomeI18nStatic(){
  document.querySelectorAll('[data-home-i18n]').forEach(el=>{ el.textContent=ht(el.dataset.homeI18n); });
  document.querySelectorAll('[data-home-i18n-placeholder]').forEach(el=>{ el.setAttribute('placeholder', ht(el.dataset.homeI18nPlaceholder)); });
}

const RETAIL_UI_I18N={
  ja:{
    marketFilterTitle:'市場・小売を探す', marketSearchPlaceholder:'例：Carrefour / 台湾 / コンビニ',
    allSourceTypes:'出典区分すべて', officialSemi:'公式・準公式', referenceValue:'参考値', presenceConfirmed:'展開確認',
    allStoreCounts:'店舗数の有無すべて', withStoreCount:'店舗数あり', withoutStoreCount:'店舗数未確定',
    clearFilters:'条件をクリア', showMore:'さらに表示', filteredRetailSummary:'絞り込み範囲の小売集計',
    overallRetailSummary:'全体の小売集計', retailSummaryOf:'の小売集計',
    retailProfiles:'小売店プロフィール', matched:'条件に一致', withText:'本文あり',
    showMoreProfiles:'プロフィールをさらに表示', source:'出典', detailProfile:'詳細プロフィール',
    storeCountUnlinked:'店舗数リンク未設定', retailChain:rui('retailChain'), retailData:rui('retailData'),
    retailStore:'小売店', retail:'小売', profile:'プロフィール', profileBody:'店舗本文',
    storeCountLink:'店舗数リンク', profileCountryHeadLead:'Excel本文を店舗メモ・歴史情報として表示',
    storeCountNote:'店舗数列は使用せず、本文中の数値は原文の一部として保持しています。上部の店舗数は、既存小売正本と照合できた場合だけ表示します。',
    countSuffix:'件'
  },
  en:{
    marketFilterTitle:'Search market / retail', marketSearchPlaceholder:'Example: Carrefour / Taiwan / convenience store',
    allSourceTypes:'All source types', officialSemi:'Official / semi-official', referenceValue:'Reference value', presenceConfirmed:'Presence confirmed',
    allStoreCounts:'All store-count statuses', withStoreCount:'With store count', withoutStoreCount:'Store count not fixed',
    clearFilters:'Clear filters', showMore:'Show more', filteredRetailSummary:'Retail summary for current filters',
    overallRetailSummary:'Overall retail summary', retailSummaryOf:' retail summary',
    retailProfiles:'Retail profiles', matched:'matched', withText:'with text',
    showMoreProfiles:'Show more profiles', source:'Source', detailProfile:'Detailed profile',
    storeCountUnlinked:'Store-count link not set', retailChain:'Retail chain', retailData:'Retail data',
    retailStore:'Retail store', retail:'Retail', profile:'Profile', profileBody:'Store profile text',
    storeCountLink:'Store-count link', profileCountryHeadLead:'Shows Excel text as store notes and history information',
    storeCountNote:'The store-count column is not used. Numbers inside body text are kept as part of the original text. The store count above is shown only when it can be matched with the existing retail master.',
    countSuffix:''
  },
  zh_tw:{
    marketFilterTitle:'搜尋市場／零售', marketSearchPlaceholder:'例：Carrefour / 台灣 / 便利商店',
    allSourceTypes:'所有來源區分', officialSemi:'官方／準官方', referenceValue:'參考值', presenceConfirmed:'展開資訊',
    allStoreCounts:'所有店舖數狀態', withStoreCount:'有店舖數', withoutStoreCount:'店舖數未確定',
    clearFilters:'清除條件', showMore:'顯示更多', filteredRetailSummary:'篩選範圍的零售彙總',
    overallRetailSummary:'整體零售彙總', retailSummaryOf:'的零售彙總',
    retailProfiles:'零售店簡介', matched:'符合條件', withText:'有本文',
    showMoreProfiles:'顯示更多簡介', source:'來源', detailProfile:'詳細簡介',
    storeCountUnlinked:'店舖數連結未設定', retailChain:'零售連鎖', retailData:'零售資料',
    retailStore:'零售店', retail:'零售', profile:'簡介', profileBody:'店舖本文',
    storeCountLink:'店舖數連結', profileCountryHeadLead:'將 Excel 本文作為店舖備註與歷史資訊顯示',
    storeCountNote:'不使用店舖數欄位。本文中的數字保留為原文內容的一部分。上方店舖數僅在可與既有零售正本比對時顯示。',
    countSuffix:'件'
  },
  zh_cn:{
    marketFilterTitle:'搜索市场／零售', marketSearchPlaceholder:'例：Carrefour / 台湾 / 便利店',
    allSourceTypes:'所有来源区分', officialSemi:'官方／准官方', referenceValue:'参考值', presenceConfirmed:'展开信息',
    allStoreCounts:'所有门店数状态', withStoreCount:'有门店数', withoutStoreCount:'门店数未确定',
    clearFilters:'清除条件', showMore:'显示更多', filteredRetailSummary:'筛选范围的零售汇总',
    overallRetailSummary:'整体零售汇总', retailSummaryOf:'的零售汇总',
    retailProfiles:'零售店简介', matched:'符合条件', withText:'有正文',
    showMoreProfiles:'显示更多简介', source:'来源', detailProfile:'详细简介',
    storeCountUnlinked:'门店数链接未设置', retailChain:'零售连锁', retailData:'零售数据',
    retailStore:'零售店', retail:'零售', profile:'简介', profileBody:'门店正文',
    storeCountLink:'门店数链接', profileCountryHeadLead:'将 Excel 正文作为门店备注与历史信息显示',
    storeCountNote:'不使用门店数字段。正文中的数字保留为原文内容的一部分。上方门店数仅在可与既有零售主数据比对时显示。',
    countSuffix:'条'
  }
};
function rui(key){ return (RETAIL_UI_I18N[lang] && RETAIL_UI_I18N[lang][key]) || RETAIL_UI_I18N.ja[key] || key; }

function retailDisplayName(value){
  let text=String(value || '').trim();
  if(!text) return '';
  if(lang!=='ja') return text;
  const replacements=[
    [/7[\-\s]?ELEVEN/gi,'セブンイレブン'],
    [/Seven\s*&\s*I\s*Holdings/gi,'セブン＆アイ・ホールディングス'],
    [/FamilyMart/gi,'ファミリーマート'],
    [/Circle\s*K/gi,'サークルK'],
    [/Carrefour/gi,'カルフール'],
    [/Costco/gi,'コストコ'],
    [/LOTTE\s*Mart/gi,'ロッテマート'],
    [/Lotte\s*Mart/gi,'ロッテマート'],
    [/Lotte\s*Grosir/gi,'ロッテグロッサー'],
    [/Watsons/gi,'ワトソンズ'],
    [/Guardian/gi,'ガーディアン']
  ];
  replacements.forEach(([pat,rep])=>{ text=text.replace(pat,rep); });
  return text;
}

const RETAIL_HOME_I18N={
  ja:{dbTitle:'国別小売データベース', dbHomeLead:'小売チェーン、店舗数、プロフィール本文、出典を国ごとに確認できます。', more:'詳しく見る →'},
  en:{dbTitle:'Country Retail Database', dbHomeLead:'Review retail chains, store counts, profile text, and sources by country.', more:'View details →'},
  zh_tw:{dbTitle:'各國零售資料庫', dbHomeLead:'可依國家查看零售連鎖、店舖數、簡介本文與來源。', more:'查看詳情 →'},
  zh_cn:{dbTitle:'各国零售数据库', dbHomeLead:'可按国家查看零售连锁、门店数、简介正文与来源。', more:'查看详情 →'}
};
function rt(key){ return (RETAIL_HOME_I18N[lang] && RETAIL_HOME_I18N[lang][key]) || RETAIL_HOME_I18N.ja[key] || key; }
function applyRetailHomeI18nStatic(){
  document.querySelectorAll('[data-retail-i18n]').forEach(el=>{ el.textContent=rt(el.dataset.retailI18n); });
  document.querySelectorAll('[data-retail-i18n-aria]').forEach(el=>{ el.setAttribute('aria-label', rt(el.dataset.retailI18nAria)); });
}

/* V161: country school meals database UI translation batch 1 */


const SCHOOL_I18N={
  ja:{
    dbTitle:'国別学校給食データベース', dbLead:'国ごとに、学校給食制度、中央厨房、調理設備業者、入札・事例リンクを整理しています。', dbHomeLead:'学校給食制度、中央厨房、調理設備業者、入札・事例リンクを国ごとに確認できます。', more:'詳しく見る →', badge:'SCHOOL MEALS DATABASE',
    countriesAreas:'国・地域', vendorsAvailable:'調理設備業者あり', vendors:'調理設備業者', centralInfo:'中央厨房情報', centralKitchen:'中央厨房', yes:'あり', needsReview:'要確認', facility:'制度・施設', operators:'運営別枠', sources:'出典', detail:'詳細', countryCards:'国別カード', all:'すべて', search:'国・会社・設備を検索', regionAll:'地域すべて', sourceOrganizing:'出典リンクは整理中です。', noVendor:'学校給食と調理設備業者を直接つなぐ採用行はまだありません。', cases:'案件・掲載先', targetEquipment:'対象設備', company:'会社', operatorPolicyFacility:'中央厨房運営・政策・施設', facilityMemo:'制度・施設メモ', schoolMeal:'学校給食', schoolData:'学校給食データ', publicIndicators:'給食対象児童・生徒数、給食提供率など、登録済みの公開指標を表示しています。', practicalInfo:'国別実務情報', dataAvailable:'学校給食データあり', childrenServed:'給食対象', referenceRate:'参考率', noDetailData:'詳細データは未収録です。', countryArea:'国・地域', programs:'制度・施設', childrenStudents:'給食対象児童・生徒数', coverageRate:'給食提供率（参考）', menuQuality:'メニュー品質スコア', notCollected:'未収録', sourceNotSet:'出典未設定', showMore:'さらに表示', countSuffix:'件'
  },
  en:{
    dbTitle:'Country School Meals Database', dbLead:'Organizes school meal systems, central kitchens, cooking equipment suppliers, tenders, and case links by country.', dbHomeLead:'Review school meal systems, central kitchens, cooking equipment suppliers, tenders, and case links by country.', more:'View details →', badge:'SCHOOL MEALS DATABASE',
    countriesAreas:'countries/areas', vendorsAvailable:'with equipment suppliers', vendors:'cooking equipment suppliers', centralInfo:'central kitchen info', centralKitchen:'Central kitchen', yes:'Yes', needsReview:'Needs review', facility:'Systems/facilities', operators:'Operators', sources:'Sources', detail:'Detail', countryCards:'Country cards', all:'All', search:'Search country, company, or equipment', regionAll:'All regions', sourceOrganizing:'Source links are being organized.', noVendor:'No adopted rows directly connecting school meals and cooking equipment suppliers yet.', cases:'Project / listing', targetEquipment:'Target equipment', company:'Company', operatorPolicyFacility:'Central kitchen operators, policy, and facilities', facilityMemo:'System / facility notes', schoolMeal:'School meals', schoolData:'School meal data', publicIndicators:'Shows registered public indicators such as children/students covered and coverage rate.', practicalInfo:'Country practical info', dataAvailable:'School meal data available', childrenServed:'Children/students served', referenceRate:'reference rate', noDetailData:'Detailed data is not recorded yet.', countryArea:'Country/area', programs:'Systems/facilities', childrenStudents:'Children/students covered by school meals', coverageRate:'School meal coverage rate (reference)', menuQuality:'Menu quality score', notCollected:'Not recorded', sourceNotSet:'Source not set', showMore:'Show more', countSuffix:''
  },
  zh_tw:{
    dbTitle:'各國學校供餐資料庫', dbLead:'依國家整理學校供餐制度、中央廚房、調理設備業者、標案與案例連結。', dbHomeLead:'可依國家查看學校供餐制度、中央廚房、調理設備業者、標案與案例連結。', more:'查看詳情 →', badge:'SCHOOL MEALS DATABASE',
    countriesAreas:'國家／地區', vendorsAvailable:'有調理設備業者', vendors:'調理設備業者', centralInfo:'中央廚房資訊', centralKitchen:'中央廚房', yes:'有', needsReview:'待確認', facility:'制度／設施', operators:'營運別項', sources:'來源', detail:'詳情', countryCards:'國別卡片', all:'全部', search:'搜尋國家、公司或設備', regionAll:'所有地區', sourceOrganizing:'來源連結整理中。', noVendor:'目前尚無直接連結學校供餐與調理設備業者的採用資料。', cases:'案件／刊載處', targetEquipment:'對象設備', company:'公司', operatorPolicyFacility:'中央廚房營運、政策與設施', facilityMemo:'制度／設施備註', schoolMeal:'學校供餐', schoolData:'學校供餐資料', publicIndicators:'顯示已收錄的公開指標，例如供餐對象兒童／學生數與供餐率。', practicalInfo:'國別實務資訊', dataAvailable:'已有學校供餐資料', childrenServed:'供餐對象', referenceRate:'參考率', noDetailData:'詳細資料尚未收錄。', countryArea:'國家／地區', programs:'制度／設施', childrenStudents:'學校供餐對象兒童／學生數', coverageRate:'學校供餐率（參考）', menuQuality:'菜單品質分數', notCollected:'未收錄', sourceNotSet:'來源未設定', showMore:'顯示更多', countSuffix:'件'
  },
  zh_cn:{
    dbTitle:'各国学校供餐数据库', dbLead:'按国家整理学校供餐制度、中央厨房、调理设备供应商、招标与案例链接。', dbHomeLead:'可按国家查看学校供餐制度、中央厨房、调理设备供应商、招标与案例链接。', more:'查看详情 →', badge:'SCHOOL MEALS DATABASE',
    countriesAreas:'国家／地区', vendorsAvailable:'有调理设备供应商', vendors:'调理设备供应商', centralInfo:'中央厨房信息', centralKitchen:'中央厨房', yes:'有', needsReview:'待确认', facility:'制度／设施', operators:'运营别项', sources:'来源', detail:'详情', countryCards:'国家卡片', all:'全部', search:'搜索国家、公司或设备', regionAll:'所有地区', sourceOrganizing:'来源链接整理中。', noVendor:'目前尚无直接连接学校供餐与调理设备供应商的采用资料。', cases:'项目／刊载处', targetEquipment:'对象设备', company:'公司', operatorPolicyFacility:'中央厨房运营、政策与设施', facilityMemo:'制度／设施备注', schoolMeal:'学校供餐', schoolData:'学校供餐数据', publicIndicators:'显示已收录的公开指标，例如供餐对象儿童／学生数与供餐率。', practicalInfo:'国家实务信息', dataAvailable:'已有学校供餐数据', childrenServed:'供餐对象', referenceRate:'参考率', noDetailData:'详细数据尚未收录。', countryArea:'国家／地区', programs:'制度／设施', childrenStudents:'学校供餐对象儿童／学生数', coverageRate:'学校供餐率（参考）', menuQuality:'菜单质量分数', notCollected:'未收录', sourceNotSet:'来源未设置', showMore:'显示更多', countSuffix:'条'
  }
};
function st(key){ return (SCHOOL_I18N[lang] && SCHOOL_I18N[lang][key]) || SCHOOL_I18N.ja[key] || key; }
function countText(n){ return lang==='en' ? String(n) : `${n}${st('countSuffix')}`; }
function applySchoolI18nStatic(){
  document.querySelectorAll('[data-school-i18n]').forEach(el=>{ el.textContent=st(el.dataset.schoolI18n); });
  document.querySelectorAll('[data-school-i18n-aria]').forEach(el=>{ el.setAttribute('aria-label', st(el.dataset.schoolI18nAria)); });
}
function schoolDisplayText(value){
  return String(value || '')
    .replaceAll('要追加調査','追加調査中')
    .replaceAll('未整理','情報整理中')
    .replaceAll('未確認','情報整理中')
    .replaceAll('後回し','追加調査中')
    .replaceAll('機械設備業者','調理設備業者')
    .replaceAll('機械設備会社','調理設備業者')
    .replaceAll('機械設備','調理設備');
}
function schoolValidUrl(url){ return /^https?:\/\//.test(String(url || '').trim()); }
function schoolCkRecords(){ return (schoolCentralKitchenData?.records || []); }
function schoolCkRecordForEntity(entityId){ return schoolCkRecords().find(r=>r.entity_id===entityId) || null; }
function schoolCkSummary(){
  const rows=schoolCkRecords();
  const vendorCountries=rows.filter(r=>(r.equipment_vendor_count||0)>0).length;
  const centralCountries=rows.filter(r=>r.has_central_kitchen || (r.program_count||0)>0 || (r.operator_count||0)>0).length;
  const vendors=rows.reduce((sum,r)=>sum+(r.equipment_vendor_count||0),0);
  const operators=rows.reduce((sum,r)=>sum+(r.operator_count||0),0);
  return {countries:rows.length, vendorCountries, centralCountries, vendors, operators};
}
function schoolCkSearchText(rec){
  const e=entities.find(x=>x.entity_id===rec.entity_id) || {};
  const vendors=(rec.equipment_vendors||[]).map(v=>[v.company_name,v.vendor_type,(v.equipment_set||[]).join(' ')].join(' ')).join(' ');
  const ops=(rec.operators||[]).map(o=>[o.name,o.operator_type,o.target].join(' ')).join(' ');
  const programs=(rec.programs||[]).map(p=>[p.program_name,p.operator,p.target,p.central_kitchen_model,p.equipment_status_note].join(' ')).join(' ');
  return [rec.name_ja,rec.name_en,rec.entity_id,e.region,e.subregion,vendors,ops,programs].filter(Boolean).join(' ').toLowerCase();
}
function filteredSchoolRecords(){
  const ckRows=schoolCkRecords();
  if(!ckRows.length){
    let rows=(schoolMealsData?.records || []).slice();
    const q=String(schoolFilters.q||'').trim().toLowerCase();
    if(q) rows=rows.filter(r=>schoolSearchText(r).includes(q));
    if(schoolFilters.region && schoolFilters.region!=='all') rows=rows.filter(r=>{ const e=entities.find(x=>x.entity_id===r.entity_id); return e?.region===schoolFilters.region; });
    return rows.sort((a,b)=>String(a.name_ja||a.name_en||a.entity_id).localeCompare(String(b.name_ja||b.name_en||b.entity_id),'ja'));
  }
  let rows=ckRows.slice();
  const q=String(schoolFilters.q||'').trim().toLowerCase();
  const mode=schoolFilters.mode || 'all';
  if(q) rows=rows.filter(r=>schoolCkSearchText(r).includes(q));
  if(schoolFilters.region && schoolFilters.region!=='all') rows=rows.filter(r=>{ const e=entities.find(x=>x.entity_id===r.entity_id); return (r.region || e?.region)===schoolFilters.region; });
  if(mode==='vendors') rows=rows.filter(r=>(r.equipment_vendor_count||0)>0);
  if(mode==='central') rows=rows.filter(r=>r.has_central_kitchen || (r.program_count||0)>0 || (r.operator_count||0)>0);
  return rows.sort((a,b)=>{ const av=(a.equipment_vendor_count||0), bv=(b.equipment_vendor_count||0); if(bv!==av) return bv-av; return String(a.name_ja||a.name_en||a.entity_id).localeCompare(String(b.name_ja||b.name_en||b.entity_id),'ja'); });
}
function updateSchoolFilter(kind, value){ schoolFilters[kind]=value || 'all'; schoolVisibleLimit=24; renderSchoolPanel(); }
function schoolCkVendorPreview(rec){
  const vendors=(rec.equipment_vendors||[]).slice(0,4).map(v=>v.company_name).filter(Boolean);
  if(vendors.length) return vendors.join('、') + ((rec.equipment_vendors||[]).length>4 ? ' ほか' : '');
  const ops=(rec.operators||[]).slice(0,3).map(o=>o.name).filter(Boolean);
  if(ops.length) return ops.join('、');
  const p=(rec.programs||[])[0];
  return p?.program_name || `${st('facility')} ${st('yes')}`;
}
function schoolRecordCard(rec){
  if(!schoolCkRecords().length){
    const e=entities.find(x=>x.entity_id===rec.entity_id) || {};
    const main=schoolMetricValueText(rec.school_meals_total_children,'school_meals_total_children');
    const pct=schoolMetricValueText(rec.school_meal_children_per_primary_enrollment_pct,'school_meal_children_per_primary_enrollment_pct');
    const has=rec.school_meals_total_children || rec.school_meal_children_per_primary_enrollment_pct || rec.school_meal_menu_gdqs_score;
    const excerpt=has ? `<p class="retail-note retail-profile-excerpt">${safe(main!==st('notCollected')?`${st('childrenServed')}：${main}`:st('dataAvailable'))}${pct!==st('notCollected')?` / ${st('referenceRate')}：${safe(pct)}`:''}</p>` : `<p class="retail-note retail-profile-excerpt">${safe(st('noDetailData'))}</p>`;
    return `<article class="market-data-card retail-profile-card school-profile-card"><div class="retail-card-top"><div><strong>${flagText(e)} ${safe(rec.name_ja || nameOf(e) || rec.entity_id)}</strong><span class="retail-profile-subname">${safe(rec.name_en || rec.iso3 || '')}</span></div><span class="retail-category-pill">${safe(st('schoolMeal'))}</span></div><div class="retail-card-facts"><span class="retail-store-pill">${safe(main)}</span>${pct!==st('notCollected')?`<span class="retail-info-chip">${safe(pct)}</span>`:''}</div>${excerpt}<div class="retail-source-line"><button class="retail-more-button" type="button" onclick="openSchoolProfile('${safe(rec.entity_id)}')">${safe(st('detail'))}</button></div></article>`;
  }
  const e=entities.find(x=>x.entity_id===rec.entity_id) || {};
  const vendorCount=rec.equipment_vendor_count||0;
  const operatorCount=rec.operator_count||0;
  const programCount=rec.program_count||0;
  const summary=schoolCkVendorPreview(rec);
  return `<article class="market-data-card retail-profile-card school-profile-card school-ck-card">
    <div class="retail-card-top"><div><strong>${flagText(e)} ${safe(rec.name_ja || nameOf(e) || rec.entity_id)}</strong><span class="retail-profile-subname">${safe(rec.name_en || rec.entity_id)}</span></div><span class="retail-category-pill">${safe(st('schoolMeal'))}</span></div>
    <div class="retail-card-facts"><span class="retail-store-pill">${safe(st('vendors'))} ${safe(vendorCount)}</span><span class="retail-info-chip">${safe(st('centralKitchen'))} ${safe(programCount || operatorCount ? st('yes') : st('needsReview'))}</span></div>
    <p class="retail-note retail-profile-excerpt">${safe(summary)}</p>
    <div class="school-ck-mini-grid"><span>${safe(st('facility'))} ${safe(programCount)}</span><span>${safe(st('operators'))} ${safe(operatorCount)}</span><span>${safe(st('sources'))} ${safe((rec.sources||[]).length)}</span></div>
    <div class="retail-source-line"><button class="retail-more-button" type="button" onclick="openSchoolProfile('${safe(rec.entity_id)}')">${safe(st('detail'))}</button></div>
  </article>`;
}
function renderSchoolPanel(){
  const panel=document.getElementById('schoolPanel');
  ...
  if(!schoolCkRecords().length){
    ...
    const rows=filteredSchoolRecords();
    const visible=rows.slice(0, schoolVisibleLimit);
    panel.innerHTML=`<section class="retail-profile-section school-profile-section"><div class="retail-profile-section-head"><strong>${safe(st('countryArea'))}</strong><span>${rows.length.toLocaleString()}${safe(st('countSuffix'))}</span></div><div class="company-home-filter-row school-filter-row"><label class="company-search-box"><span>⌕</span><input type="search" value="${safe(schoolFilters.q==='all'?'':schoolFilters.q)}" placeholder="${safe(t('search'))}" oninput="updateSchoolFilter('q', this.value)"></label><select onchange="updateSchoolFilter('region', this.value)"><option value="all">${safe(st('regionAll'))}</option>${regions.map(r=>`<option value="${safe(r)}" ${schoolFilters.region===r?'selected':''}>${safe(REGION_LABELS[r] || r)}</option>`).join('')}</select></div><div class="market-data-grid compact">${visible.map(schoolRecordCard).join('')}</div>${rows.length>visible.length?`<div class="market-filter-actions"><button onclick="schoolVisibleLimit+=24; renderSchoolPanel();">${safe(st('showMore'))}</button></div>`:''}</section>`;
    return;
  }
  const summary=schoolCkSummary();
  const regions=[...new Set(entities.map(e=>e.region).filter(Boolean))].sort();
  const rows=filteredSchoolRecords();
  const visible=rows.slice(0, schoolVisibleLimit);
  panel.innerHTML=`<section class="retail-profile-section school-profile-section school-ck-section">
    <div class="school-ck-hero"><div><span class="pill light">${safe(st('badge'))}</span><h3>${safe(st('dbTitle'))}</h3><p>${safe(st('dbLead'))}</p></div><div class="school-ck-stat-grid"><article><strong>${safe(summary.countries)}</strong><span>${safe(st('countriesAreas'))}</span></article><article><strong>${safe(summary.vendorCountries)}</strong><span>${safe(st('vendorsAvailable'))}</span></article><article><strong>${safe(summary.vendors)}</strong><span>${safe(st('vendors'))}</span></article><article><strong>${safe(summary.centralCountries)}</strong><span>${safe(st('centralInfo'))}</span></article></div></div>
    <div class="company-home-filter-row school-filter-row"><label class="company-search-box"><span>⌕</span><input type="search" value="${safe(schoolFilters.q==='all'?'':schoolFilters.q)}" placeholder="${safe(st('search'))}" oninput="updateSchoolFilter('q', this.value)"></label><select onchange="updateSchoolFilter('region', this.value)"><option value="all">${safe(st('regionAll'))}</option>${regions.map(r=>`<option value="${safe(r)}" ${schoolFilters.region===r?'selected':''}>${safe(REGION_LABELS[r] || r)}</option>`).join('')}</select><select onchange="updateSchoolFilter('mode', this.value)"><option value="all" ${(schoolFilters.mode||'all')==='all'?'selected':''}>${safe(st('all'))}</option><option value="vendors" ${schoolFilters.mode==='vendors'?'selected':''}>${safe(st('vendorsAvailable'))}</option><option value="central" ${schoolFilters.mode==='central'?'selected':''}>${safe(st('centralInfo'))}</option></select></div>
    <div class="retail-profile-section-head"><strong>${safe(st('countryCards'))}</strong><span>${rows.length.toLocaleString()}${safe(st('countSuffix'))}</span></div>
    <div class="market-data-grid compact school-ck-grid">${visible.map(schoolRecordCard).join('')}</div>
    ${rows.length>visible.length?`<div class="market-filter-actions"><button onclick="schoolVisibleLimit+=24; renderSchoolPanel();">${safe(st('showMore'))}</button></div>`:''}
  </section>`;
}
function linkListHtml(items){
  const links=(items||[]).filter(x=>x && schoolValidUrl(x.url)).slice(0,8);
  if(!links.length) return `<p>${safe(st('sourceOrganizing'))}</p>`;
  return `<div class="school-ck-source-list">${links.map(x=>`<a href="${safe(x.url)}" target="_blank" rel="noopener">${safe(x.label || x.url)}</a>`).join('')}</div>`;
}
function vendorListHtml(rec){
  const vendors=(rec.equipment_vendors||[]);
  if(!vendors.length) return `<p>${safe(st('noVendor'))}</p>`;
  return `<div class="school-ck-detail-list">${vendors.map(v=>{ const cases=(v.cases||[]).slice(0,3).map(c=>`<li><strong>${safe(c.project_or_case || st('cases'))}</strong><span>${safe(c.equipment || (v.equipment_set||[]).slice(0,2).join('、') || st('targetEquipment'))}</span>${c.evidence?`<em>${safe(c.evidence)}</em>`:''}</li>`).join(''); return `<article><h4>${safe(v.company_name)}</h4><div class="retail-card-facts">${v.vendor_type?`<span class="retail-info-chip">${safe(v.vendor_type)}</span>`:''}${v.company_url?`<a class="retail-more-button" href="${safe(v.company_url)}" target="_blank" rel="noopener">${safe(st('company'))}</a>`:''}</div><ul>${cases}</ul></article>`; }).join('')}</div>`;
}
function operatorListHtml(rec){
  const ops=(rec.operators||[]);
  if(!ops.length) return '';
  return `<article class="market-note-card retail-profile-body"><h4>${safe(st('operatorPolicyFacility'))}</h4><div class="school-ck-source-list">${ops.map(o=>`${schoolValidUrl(o.evidence_url || o.url)?`<a href="${safe(o.evidence_url || o.url)}" target="_blank" rel="noopener">${safe(schoolDisplayText(o.name))}${o.operator_type?`｜${safe(schoolDisplayText(o.operator_type))}`:''}</a>`:`<span>${safe(schoolDisplayText(o.name))}${o.operator_type?`｜${safe(schoolDisplayText(o.operator_type))}`:''}</span>`}`).join('')}</div></article>`;
}
function programListHtml(rec){
  const programs=(rec.programs||[]);
  if(!programs.length) return '';
  return `<article class="market-note-card retail-profile-body"><h4>${safe(st('facilityMemo'))}</h4>${programs.slice(0,3).map(p=>`<div class="school-ck-program"><strong>${safe(schoolDisplayText(p.program_name))}</strong><p>${safe(schoolDisplayText(p.central_kitchen_model || p.target || ''))}</p><small>${safe(schoolDisplayText([p.scale,p.delivery_temperature_note,p.equipment_status_note].filter(Boolean).join(' / ')))}</small></div>`).join('')}</article>`;
}
function schoolInfoRows(rec){
  if(schoolCkRecords().length){
    const rows=[[st('countryArea'), rec.name_ja],[st('vendors'), countText(rec.equipment_vendor_count||0)],[st('programs'), countText(rec.program_count||0)],[st('operators'), countText(rec.operator_count||0)]];
    return `<div class="retail-profile-info-grid">${rows.map(([k,v])=>`<div><span>${safe(k)}</span><strong>${safe(v)}</strong></div>`).join('')}</div>`;
  }
  const e=entities.find(x=>x.entity_id===rec.entity_id) || {};
  const rows=[[st('countryArea'), rec.name_ja || nameOf(e)],[st('childrenStudents'), schoolMetricValueText(rec.school_meals_total_children,'school_meals_total_children')],[st('coverageRate'), schoolMetricValueText(rec.school_meal_children_per_primary_enrollment_pct,'school_meal_children_per_primary_enrollment_pct')],[st('menuQuality'), rec.school_meal_menu_gdqs_score ? schoolMetricValueText(rec.school_meal_menu_gdqs_score,'school_meal_menu_gdqs_score') : '']].filter(x=>x[1] && x[1]!==st('notCollected'));
  if(!rows.length) return '';
  return `<div class="retail-profile-info-grid">${rows.map(([k,v])=>`<div><span>${safe(k)}</span><strong>${safe(v)}</strong></div>`).join('')}</div>`;
}
function openSchoolProfile(entityId){
  const ck=schoolCkRecordForEntity(entityId);
  const e=entities.find(x=>x.entity_id===entityId) || {};
  if(ck){
    document.getElementById('detailContent').innerHTML=`<div class="detail-header retail-profile-detail-header"><div class="detail-flag">${flagMarkup(e,'flag-img-detail')}</div><div class="detail-title-block"><h2>${safe(nameOf(e) || ck.name_ja || st('schoolMeal'))}</h2><p class="country-sub">${safe(st('schoolMeal'))}</p><div class="detail-mini-tabs"><span>${safe(st('facility'))}</span><span>${safe(st('vendors'))}</span><span>${safe(st('sources'))}</span></div></div></div><section class="detail-section retail-profile-detail school-ck-detail"><h3>${flagText(e)} ${safe(nameOf(e) || ck.name_ja)}｜${safe(st('schoolMeal'))}</h3>${schoolInfoRows(ck)}${programListHtml(ck)}<article class="market-note-card retail-profile-body"><h4>${safe(st('vendors'))}</h4>${vendorListHtml(ck)}</article>${operatorListHtml(ck)}<article class="market-note-card retail-profile-body"><h4>${safe(st('sources'))}</h4>${linkListHtml(ck.sources)}</article></section>`;
    const dialog=document.getElementById('detailDialog'); if(!dialog.open) dialog.showModal(); return;
  }
  const rec=schoolMealRecordForEntity(entityId); if(!rec) return;
  const sources=[rec.school_meals_total_children,rec.school_meal_children_per_primary_enrollment_pct,rec.school_meal_menu_gdqs_score].map(schoolMetricSourceText).filter(Boolean);
  const sourceText=[...new Set(sources)].slice(0,3).join('<br>') || st('sourceNotSet');
  document.getElementById('detailContent').innerHTML=`<div class="detail-header retail-profile-detail-header"><div class="detail-flag">${flagMarkup(e,'flag-img-detail')}</div><div class="detail-title-block"><h2>${safe(nameOf(e) || rec.name_ja || st('schoolMeal'))}</h2><p class="country-sub">${safe(st('schoolMeal'))}</p><div class="detail-mini-tabs"><span>${safe(st('countryArea'))}</span><span>${safe(st('schoolMeal'))}</span><span>${safe(st('sources'))}</span></div></div></div><section class="detail-section retail-profile-detail"><h3>${flagText(e)} ${safe(nameOf(e) || rec.name_ja)}｜${safe(st('schoolMeal'))}</h3>${schoolInfoRows(rec)}<article class="market-note-card retail-profile-body"><h4>${safe(st('schoolData'))}</h4><p>${safe(st('publicIndicators'))}</p></article><article class="market-note-card retail-profile-body"><h4>${safe(st('sources'))}</h4><p>${sourceText}</p></article></section>`;
  const dialog=document.getElementById('detailDialog'); if(!dialog.open) dialog.showModal();
}
function schoolMealCardsForEntity(entity){
  const ck=schoolCkRecordForEntity(entity.entity_id);
  const dataCards=schoolCardsFromData(entity);
  let ckCards='';
  if(ck){
    ckCards=`<article class="market-data-card school-summary-card school-ready-card"><strong>学校給食データ</strong><span>${safe((ck.equipment_vendor_count||0)+'件')}</span><em>調理設備業者 / 制度・施設 ${(ck.program_count||0)}件</em></article>`;
  }
  if(dataCards || ckCards) return dataCards + ckCards;
  return MARKET_DOMAINS.school.items.map(x=>notCollectedCard(x[0],x[1])).join('');
}
function schoolSummaryCards(){
  const ck=schoolCkSummary();
  if(ck.countries){
    return `<article class="market-data-card school-summary-card school-ready-card"><strong>学校給食</strong><span>${safe(ck.countries)}国・地域</span><em>調理設備業者 ${safe(ck.vendors)}件</em></article>
    <article class="market-data-card school-summary-card school-ready-card"><strong>国別実務情報</strong><span>${safe(ck.vendorCountries)}国</span><em>調理設備業者あり</em></article>`;
  }
  const s=schoolStatusSummary();
  const hasSchools=!!(schoolMealRankings?.school_meals_total_schools?.items || []).length;
  const status=s.any ? `値あり ${s.any}/${s.total}` : '準備中';
...
async function boot(){
  try{
    const [e,r,retail,retailPresence,retailProfiles,cvsSummary,cvsCompanies,cvsFactories,cvsCandidates,cvsTasks,rice,riceRanks,schoolMeals,schoolRanks,schoolCk,priority4,wdiMeta,japanRel,japanRanks,jpRestaurantOverview,jpResidentsOverview]=await Promise.all([
      ...
      fetch('data/market_base_cvs_vendor_country_summary_CURRENT.json').then(x=>x.json()).catch(()=>null),
      fetch('data/market_base_cvs_vendor_companies_CURRENT.json').then(x=>x.json()).catch(()=>null),
      fetch('data/market_base_cvs_vendor_factories_CURRENT.json').then(x=>x.json()).catch(()=>null),
      fetch('data/market_base_cvs_vendor_research_candidates_CURRENT.json').then(x=>x.json()).catch(()=>null),
      fetch('data/market_base_cvs_vendor_research_tasks_CURRENT.json').then(x=>x.json()).catch(()=>null),
      ...
      fetch('data/market_base_school_central_kitchens_CURRENT.json').then(x=>x.json()).catch(()=>null),
    ...
    entities=e.entities||[]; rankings=r.rankings||{}; retailData=retail || window.MARKET_BASE_RETAIL_DATA?.retail || null; retailPresenceData=retailPresence || window.MARKET_BASE_RETAIL_PRESENCE_DATA?.retail_presence || null; retailProfilesData=retailProfiles || null; cvsVendorCountrySummaryData=cvsSummary || null; cvsVendorCompaniesData=cvsCompanies || null; cvsVendorFactoriesData=cvsFactories || null; cvsVendorCandidatesData=cvsCandidates || null; cvsVendorTasksData=cvsTasks || null; riceData=rice || null; riceRankings=riceRanks?.rankings || {}; schoolMealsData=schoolMeals || null; schoolMealRankings=schoolRanks?.rankings || {}; schoolCentralKitchenData=schoolCk || window.MARKET_BASE_EMBEDDED_DATA?.school_central_kitchens || null; priority4ReadyData=priority4 || null; wdiCountryMetadata=wdiMeta || null; japanRelatedData=japanRel || null; japanRelatedRankings=japanRanks?.rankings || {}; japaneseRestaurantsOverview=jpRestaurantOverview || null; overseasJapaneseResidentsOverview=jpResidentsOverview || null; renderAll();
  }catch(err){
    if(window.MARKET_BASE_EMBEDDED_DATA){
      ...
      cvsVendorCountrySummaryData=emb.cvs_vendor_country_summary || null;
      cvsVendorCompaniesData=emb.cvs_vendor_companies || null;
      cvsVendorFactoriesData=emb.cvs_vendor_factories || null;
      cvsVendorCandidatesData=emb.cvs_vendor_candidates || null;
      cvsVendorTasksData=emb.cvs_vendor_tasks || null;
      ...
      schoolCentralKitchenData=emb.school_central_kitchens || null;
