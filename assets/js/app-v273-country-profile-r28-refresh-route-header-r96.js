...
function syncPrimaryViewUrl(view){
  try{
    ...
    const valid=['countries','compare','rankings','learn','global-search','rice','school','japan'];
    if(view==='rankings' && rankingCompareActiveTab==='compare') url.searchParams.set('view','compare');
    else if(valid.includes(view)) url.searchParams.set('view',view);
...
function setRankingCompareTab(tab, opts={}){
  ...
  if(next==='compare') window.setTimeout(()=>renderCompare(),0);
  if(opts.syncUrl!==false) syncPrimaryViewUrl('rankings');
...
function switchView(view, opts={}){
  ...
  if(targetView==='rankings') setRankingCompareTab(requestedCompare?'compare':'rankings',{syncUrl:false});
  syncPrimaryViewChrome(targetView);
  if(opts.syncUrl!==false) syncPrimaryViewUrl(targetView);
  document.documentElement.classList.remove('mb-initial-subview');
  const shouldScroll=opts.scroll!==false;
  if(shouldScroll) setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'start'}),30);
}
function showHome(){
  const homeViewContent=document.getElementById('homeViewContent');
...
function applyMarketBaseDataset(bundle){
  ...
  riceData=bundle.rice_data||null;
  riceRankings=bundle.rice_rankings?.rankings||{};
  schoolMealsData=bundle.school_meals_data||null;
  schoolMealRankings=bundle.school_meals_rankings?.rankings||{};
  priority4ReadyData=bundle.priority4_ready||null;
  wdiCountryMetadata=bundle.wdi_country_metadata||null;
  japanRelatedData=bundle.japan_related_data||null;
  japanRelatedRankings=bundle.japan_related_rankings?.rankings||{};
  japaneseRestaurantsOverview=bundle.japanese_restaurants_overview||null;
  overseasJapaneseResidentsOverview=bundle.overseas_japanese_residents_overview||null;
  renderAll();
...
async function fetchMarketBaseJson(path){
  ...
  const res=await fetch(scoped,{cache:'no-store'});
...
async function boot(){
  ...
  try{
    const [e,r,retailCountrySummary,rice,riceRanks,schoolMeals,schoolRanks,priority4,wdiMeta,japanRel,japanRanks,jpRestaurantOverview,jpResidentsOverview]=await Promise.all([
      ...
      fetchMarketBaseJson('data/market_base_rice_data_CURRENT.json'),
      fetchMarketBaseJson('data/market_base_rice_rankings_CURRENT.json'),
      fetchMarketBaseJson('data/market_base_school_meals_data_CURRENT.json'),
      fetchMarketBaseJson('data/market_base_school_meals_rankings_CURRENT.json'),
      fetchMarketBaseJson('data/market_base_priority4_ready_staging_v92.json'),
      fetchMarketBaseJson('data/market_base_wdi_country_metadata_v1.json'),
      fetchMarketBaseJson('data/market_base_japan_related_data_v1.json'),
      fetchMarketBaseJson('data/market_base_japan_related_rankings_v1.json'),
      fetchMarketBaseJson('data/market_base_japanese_restaurants_overview_v1.json'),
      fetchMarketBaseJson('data/market_base_overseas_japanese_residents_overview_v1.json')
    ]);
    applyMarketBaseDataset({
      entities:e,rankings:r,retail_country_card_summary:retailCountrySummary,
      rice_data:rice,rice_rankings:riceRanks,school_meals_data:schoolMeals,
      school_meals_rankings:schoolRanks,priority4_ready:priority4,
      wdi_country_metadata:wdiMeta,japan_related_data:japanRel,
      japan_related_rankings:japanRanks,japanese_restaurants_overview:jpRestaurantOverview,
      overseas_japanese_residents_overview:jpResidentsOverview
    });
  }catch(networkError){
...
// R113.2: Today’s Journey past-seven-days navigation, with R113.1 update behavior retained.
const MARKET_BASE_APP_VERSION = window.MARKET_BASE_BUILD?.id || 'MARKET_BASE_R113_48_SPECIALIST_CSS_OWNERSHIP_20260725';
function marketBaseVersionToken(text){
  return String(text || '').split(/\r?\n/)[0].trim();
}
function marketBaseRootPath(){
  const pathname=String(window.location.pathname || '/');
  const slash=pathname.lastIndexOf('/');
  return slash>=0 ? (pathname.slice(0,slash+1) || '/') : '/';
}
function marketBaseActiveViewForRefresh(){
  try{ return typeof activePrimaryView==='function' ? (activePrimaryView() || '') : ''; }
  catch(_){ return ''; }
}
function setMarketBaseRefreshState(working){
  document.querySelectorAll('#cacheRefreshBtn,#globalCacheRefreshBtn,[data-view-refresh]').forEach(btn=>{
    if(working){
      if(!btn.dataset.mbRefreshOriginalText) btn.dataset.mbRefreshOriginalText=btn.textContent || '更新';
      btn.disabled=true;
      btn.textContent='更新中';
      btn.setAttribute('aria-busy','true');
    }else{
      btn.disabled=false;
      btn.textContent=btn.dataset.mbRefreshOriginalText || '更新';
...
async function purgeMarketBaseLocalCache(){
  ...
  if('serviceWorker' in navigator){
    ...
    await Promise.all(regs.filter(reg=>{
      try{ return new URL(reg.scope).pathname.startsWith(marketBaseRootPath()); }
      catch(_){ return false; }
...
async function checkMarketBaseVersion(){
  ...
  try{
    ...
    if(remote===current){
      ...
      sessionStorage.removeItem('market_base_auto_refresh_target');
      return false;
    }
    const last=localStorage.getItem('market_base_last_auto_refresh_version');
    const runningTarget=sessionStorage.getItem('market_base_auto_refresh_target');
    if(last===remote || runningTarget===remote) return false;
    sessionStorage.setItem('market_base_auto_refresh_running','1');
    sessionStorage.setItem('market_base_auto_refresh_target',remote);
    ...
    const url=new URL(window.location.href);
    const activeView=marketBaseActiveViewForRefresh();
    ...
    sessionStorage.removeItem('market_base_auto_refresh_running');
    sessionStorage.removeItem('market_base_auto_refresh_target');
    setMarketBaseRefreshState(false);
    return false;
  }
}
...
async function refreshMarketBaseCache(event){
  ...
  setMarketBaseRefreshState(true);
  let target=marketBaseVersionToken(MARKET_BASE_APP_VERSION);
  try{
    if(/^https?:$/.test(window.location.protocol)){
      const response=await fetch('version.txt?manualCheck='+Date.now(),{cache:'no-store'});
      if(response.ok) target=marketBaseVersionToken(await response.text())||target;
    }
    await purgeMarketBaseLocalCache();
    sessionStorage.removeItem('market_base_auto_refresh_running');
    sessionStorage.removeItem('market_base_auto_refresh_target');
    localStorage.setItem('market_base_last_auto_refresh_version',target);
    localStorage.setItem('market_base_last_cache_refresh',new Date().toISOString());
  }catch(err){
    ...
    const url=new URL(window.location.href);
    const activeView=marketBaseActiveViewForRefresh();
    if(activeView) url.searchParams.set('view',activeView); else url.searchParams.delete('view');
    url.searchParams.set('v',target.replace(/[^A-Za-z0-9_-]/g,'_'));
...
(async function initializeMarketBase(){
  const refreshStarted=await checkMarketBaseVersion();
  if(!refreshStarted) await boot();
...
if(['countries','compare','rankings','learn','global-search','rice','school','japan'].includes(requestedView)){
