...
const prefectureReferences=Array.isArray(window.MARKET_BASE_PREFECTURE_REFERENCE_DATA?.prefectures)
  ...
  : [];
let crossDbSearchIndex=window.MARKET_BASE_CROSS_DB_SEARCH_INDEX ||
  window.MARKET_BASE_CROSS_DB_SEARCH_SUMMARY || {dbs:[]};
let crossDbSearchStatus=window.MARKET_BASE_CROSS_DB_SEARCH_INDEX?'ready':'idle';
let crossDbSearchPromise=null;
const CROSS_DB_SEARCH_INDEX_PATH='embedded-cross-db-search-index-v273-db-title-r27.js';
let rankings={};
let retailData=null;
...
function formatUnifiedRankingValue(item, domain, metric){
  if(domain==='basic'){
    if(lang==='ja') return metric==='population' ? fmtJapaneseRankingPopulation(item.value) : fmt(item.value, metric);
    return `${fmt(item.value, metric)} ${item.display_unit_en || item.unit || ''}`.trim();
  }
...
function normalizeRankingValueText(value){
  let text=String(value===null || value===undefined ? '—' : value).replace(/\s+/g,' ').trim();
  text=text
    .replace(/米ドル\s*\/\s*人/g,'米ドル／人')
    .replace(/人\s*\/\s*km²/g,'人／km²')
    .replace(/kg\s*\/\s*人\s*\/\s*年/g,'kg／人／年')
    .replace(/kcal\s*\/\s*人\s*\/\s*日/g,'kcal／人／日')
    .replace(/g\s*\/\s*人\s*\/\s*日/g,'g／人／日')
    .replace(/kg\s*\/\s*ha/g,'kg／ha');
  const units=['米ドル／人','人／km²','kg／人／年','kcal／人／日','g／人／日','kg／ha','km²','米ドル','億円','万人','人','校','日','店','件','ha','t','%'];
  units.forEach(unit=>{
    const escaped=unit.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    text=text.replace(new RegExp(`(${escaped})(?:\\s*${escaped})+$`),'$1');
  });
  return text;
}
function rankingRowHtml(item, entity, name, value, source, barMax, extraClass=''){
  const eid=safe(item?.entity_id || entity?.entity_id || '');
  const rank=safe(item?.rank ?? '');
  const displayName=safe(name || item?.name_ja || item?.name_en || entity?.names?.ja || eid || '国・地域');
  const valueText=safe(normalizeRankingValueText(value));
  const sourceText=safe(source || '出典情報あり');
  const flag=eid
    ? `<button type="button" class="flag flag-open" data-open-entity="${eid}" aria-label="${displayName}の詳細を開く">${flagMarkup(entity || {}, 'flag-img-table')}</button>`
    : `<span class="flag">${flagMarkup(entity || {}, 'flag-img-table')}</span>`;
  return `<article class="ranking-row ranking-row-v96 ranking-row-v97 has-rank-bar ${safe(extraClass)}"${rankingBarStyle(item, barMax)}><div class="rank">#${rank}</div><div class="rank-bar-cell"><div class="rank-country-head">${flag}<div class="country-name">${displayName}</div></div><div class="rank-value-band"><div class="rank-bar-bg" aria-hidden="true"></div><div class="value">${valueText}</div></div><div class="source-line">${sourceText}</div></div></article>`;
}
...
function refreshRankingMetricOptions({reset=false}={}){
  const select=document.getElementById('metricSelect');
  if(!select) return;
  const previous=reset ? 'population' : (select.value || 'population');
  select.innerHTML=rankingOptionGroupsHtml();
  select.value=previous;
  if(!select.value) select.value='population';
}
...
const renderedPrimaryViews=new Set();
const GLOBAL_SEARCH_SCOPE_LABELS={all:'すべて',countries:'国・地域',companies:'企業・ブランド',content:'商品・本文'};
const CROSS_DB_SEARCH_MIN_INTENT_LENGTH=3;
function setCrossDbSearchBusy(busy){
  const results=document.getElementById('globalSearchResults');
  if(!results)return;
  if(busy){results.setAttribute('aria-busy','true');results.dataset.mbSearchIndexLoading='true';}
  else{results.removeAttribute('aria-busy');delete results.dataset.mbSearchIndexLoading;}
}
function ensureCrossDbSearchIndex(){
  if(window.MARKET_BASE_CROSS_DB_SEARCH_INDEX){
    crossDbSearchIndex=window.MARKET_BASE_CROSS_DB_SEARCH_INDEX;
    crossDbSearchStatus='ready';
    return Promise.resolve(true);
  }
  if(crossDbSearchPromise) return crossDbSearchPromise;
  crossDbSearchStatus='loading';
  setCrossDbSearchBusy(true);
  const load=window.MarketBaseRuntime?.loadScript
  ...
  crossDbSearchPromise=Promise.resolve(load).then(()=>{
    if(!window.MARKET_BASE_CROSS_DB_SEARCH_INDEX) throw new Error('cross database search index is unavailable');
    crossDbSearchIndex=window.MARKET_BASE_CROSS_DB_SEARCH_INDEX;
    crossDbSearchStatus='ready';
    setCrossDbSearchBusy(false);
    renderGlobalDbLinks();
    return true;
  }).catch(error=>{
    crossDbSearchStatus='error';
    crossDbSearchPromise=null;
    setCrossDbSearchBusy(false);
    console.warn('MARKET BASE full search index load failed; summary remains available.',error);
    return false;
  });
  return crossDbSearchPromise;
}
function refreshGlobalSearchAfterIndex(query,options={}){
  const q=String(query||'').trim();
  if(!q||globalSearchScope==='countries') return;
  const enoughIntent=normalizeSearchText(q).length>=CROSS_DB_SEARCH_MIN_INTENT_LENGTH;
  if(!options.explicit&&!enoughIntent)return;
  if(!options.explicit&&(!navigator.onLine||document.visibilityState==='hidden'))return;
  ensureCrossDbSearchIndex().then(()=>{
    if(document.getElementById('globalSearchInput')?.value===q) renderGlobalSearch(q);
  });
}
...
function renderGlobalSearch(query){
  ...
  const q=String(query||'').trim();
  const awaitingDb=!!q&&globalSearchScope!=='countries'&&crossDbSearchStatus!=='ready';
  ...
  const loading=awaitingDb ? `<div class="global-search-empty" role="status"><strong>${crossDbSearchStatus==='error'?'専門DB検索を読み込めませんでした':'専門DB検索を読み込んでいます…'}</strong><span>国・地域の結果は先に表示しています。</span></div>` : '';
  const empty=(!awaitingDb&&!countries.length&&!dbs.length) ? `<div class="global-search-empty"><strong>「${safe(q)}」の一致はありません</strong><span>表記を短くして再検索してください。</span></div>` : '';
  el.innerHTML=q ? `<div class="global-search-summary"><strong>「${safe(q)}」の検索結果</strong><span>${safe(GLOBAL_SEARCH_SCOPE_LABELS[globalSearchScope])}｜国・地域 ${countries.length}件 / 専門DB ${awaitingDb?'読込中':dbs.length+'件'}</span></div>${countryHtml}${dbHtml}${loading}${empty}` : '<div class="global-search-empty"><strong>検索語を入力してください</strong><span>国名、地域名、企業名、ブランド名、商品名、本文を同じ窓で検索できます。</span></div>';
}
function setGlobalSearchScope(scope){
  ...
  refreshGlobalSearchAfterIndex(query,{explicit:true});
...
function openGlobalSearch(query){
  ...
  // The 5 MB full index starts only after a submitted search, never on focus.
  refreshGlobalSearchAfterIndex(q,{explicit:true});
...
const PRIMARY_ROUTE_VIEWS=new Set(['countries','compare','rankings','learn','global-search','rice','school','japan']);
const PRIMARY_ROUTE_ALIASES=Object.freeze({home:'',search:'global-search',country:'countries'});
let primaryRouteSequence=0;

function normalizedPrimaryRoute(view){
  const raw=String(view||'').toLowerCase();
  const normalized=Object.prototype.hasOwnProperty.call(PRIMARY_ROUTE_ALIASES,raw)?PRIMARY_ROUTE_ALIASES[raw]:raw;
  return PRIMARY_ROUTE_VIEWS.has(normalized)?normalized:'';
}
function primaryRouteFromLocation(){
  return normalizedPrimaryRoute(new URLSearchParams(window.location.search).get('view'));
}
function primaryRouteScrollState(){
  return Math.max(0,Math.round(window.scrollY||document.documentElement.scrollTop||0));
}
function syncPrimaryViewUrl(view,opts={}){
  try{
    const route=view==='rankings'&&rankingCompareActiveTab==='compare'?'compare':normalizedPrimaryRoute(view);
    const url=new URL(window.location.href);
    if(route)url.searchParams.set('view',route);else url.searchParams.delete('view');
    const currentRoute=primaryRouteFromLocation();
    const mode=opts.historyMode||'push';
    const stateBase=(window.history.state&&typeof window.history.state==='object')?window.history.state:{};
    if(mode==='none')return;
    if(mode==='push'&&route!==currentRoute){
      window.history.replaceState({...stateBase,mbRoute:true,mbView:currentRoute,mbScrollY:primaryRouteScrollState()},'',window.location.href);
      window.history.pushState({...stateBase,mbRoute:true,mbView:route,mbScrollY:0},'',url.toString());
    }else{
      window.history.replaceState({...stateBase,mbRoute:true,mbView:route,mbScrollY:opts.keepScroll===true?primaryRouteScrollState():0},'',url.toString());
    }
...
function activePrimaryView(){
  ...
  if(active==='rankings' && document.querySelector('[data-ranking-compare-panel="compare"]:not([hidden])')) return 'compare';
  return active;
}
let rankingCompareActiveTab='rankings';
function setRankingCompareTab(tab, opts={}){
  const next=tab==='compare'?'compare':'rankings';
  // Direct ?view=compare loads the fixed comparison data only. If the user
  // later opens the sibling Rankings tab, its three ranking files begin on
  // that first use (and vice versa) instead of being coupled to the route.
  ensureOptionalDataForView(next,{reason:'ranking-compare-tab'});
  rankingCompareActiveTab=next;
  document.querySelectorAll('[data-ranking-compare-tab]').forEach(button=>{
    const selected=button.dataset.rankingCompareTab===next;
    button.classList.toggle('active',selected);
    button.setAttribute('aria-selected',selected?'true':'false');
    button.tabIndex=selected?0:-1;
  });
  document.querySelectorAll('[data-ranking-compare-panel]').forEach(panel=>{
    const selected=panel.dataset.rankingComparePanel===next;
    panel.hidden=!selected;
    panel.classList.toggle('active',selected);
  });
  if(next==='compare'&&opts.render!==false) window.setTimeout(()=>renderCompare(),0);
  if(opts.syncUrl!==false) syncPrimaryViewUrl('rankings',opts);
  if(opts.focus===true) document.querySelector(`[data-ranking-compare-tab="${next}"]`)?.focus();
}
function renderPrimaryViewOnDemand(view){
  if(renderedPrimaryViews.has(view)) return;
  if(view==='countries') renderCountries();
  else if(view==='rankings') renderRankings();
  else if(view==='sources') renderSources();
  else if(view==='qa') renderQA();
  else if(view==='global-search'){
  ...
  renderedPrimaryViews.add(view);
}
function prefersReducedPrimaryMotion(){
  return !!window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
}
function runPrimaryViewTransition(update,opts={}){
  if(opts.transition===false||prefersReducedPrimaryMotion()||typeof document.startViewTransition!=='function'){
    update();
    return null;
  }
  try{return document.startViewTransition(update);}catch(_){update();return null;}
}
function markPrimaryRouteEnter(target){
  if(!target||prefersReducedPrimaryMotion())return;
  target.classList.remove('mb-route-enter');
  window.requestAnimationFrame(()=>{
    target.classList.add('mb-route-enter');
    target.addEventListener('animationend',()=>target.classList.remove('mb-route-enter'),{once:true});
  });
}
function scrollPrimaryRoute(target,opts={}){
  if(opts.scroll===false)return;
  window.requestAnimationFrame(()=>{
    const top=Math.max(0,Math.round((target?.getBoundingClientRect().top||0)+(window.scrollY||0)-8));
    window.scrollTo({top,left:0,behavior:'auto'});
  });
}
function switchView(view, opts={}){
  const requestedCompare=view==='compare';
  const targetView=requestedCompare?'rankings':view;
  const target=document.getElementById(targetView);
  if(!target) return false;
  ensureOptionalDataForView(requestedCompare?'compare':targetView,{reason:opts.source||'route'});
  const from=activePrimaryView();
  document.dispatchEvent(new CustomEvent('marketbase:before-viewchange',{detail:{from,to:view||targetView}}));
  const firstRender=!renderedPrimaryViews.has(targetView);
  if(firstRender){document.body.classList.add('mb-view-switching');target.setAttribute('aria-busy','true');}
  const routeSequence=++primaryRouteSequence;
  const update=()=>{
    const homeViewContent=document.getElementById('homeViewContent');
    if(homeViewContent){homeViewContent.hidden=true;homeViewContent.setAttribute('aria-hidden','true');}
    ...
    if(targetView==='rankings')setRankingCompareTab(requestedCompare?'compare':'rankings',{syncUrl:false,render:!firstRender});
    syncPrimaryViewChrome(targetView);
    document.documentElement.classList.remove('mb-initial-subview');
    markPrimaryRouteEnter(target);
    if(Number.isFinite(Number(opts.restoreScroll))){
      window.requestAnimationFrame(()=>window.scrollTo({top:Math.max(0,Number(opts.restoreScroll)),left:0,behavior:'auto'}));
    }else scrollPrimaryRoute(target,opts);
    document.dispatchEvent(new CustomEvent('marketbase:viewchange',{detail:{from,to:requestedCompare?'compare':targetView,sequence:routeSequence}}));
  };
  runPrimaryViewTransition(update,opts);
  if(opts.syncUrl!==false)syncPrimaryViewUrl(requestedCompare?'compare':targetView,{historyMode:opts.historyMode||'push'});
  if(firstRender){
    // Two frames separate route feedback from heavy list/flag generation. The
    // first frame paints the selected screen and busy state; the second does
    // the synchronous render without making the original tap feel ignored.
    window.requestAnimationFrame(()=>window.requestAnimationFrame(()=>{
      if(routeSequence!==primaryRouteSequence){
        target.removeAttribute('aria-busy');
        if(!document.querySelector('.view[aria-busy="true"]'))document.body.classList.remove('mb-view-switching');
        return;
      }
      renderPrimaryViewOnDemand(targetView);
      if(targetView==='global-search'){
        renderGlobalSearch(document.getElementById('globalSearchInput')?.value || '');
        updateGlobalSearchScopeButtons();
        renderGlobalDbLinks();
      }
      if(requestedCompare)renderCompare();
      target.removeAttribute('aria-busy');
      document.body.classList.remove('mb-view-switching');
      document.dispatchEvent(new CustomEvent('marketbase:viewrendered',{detail:{view:requestedCompare?'compare':targetView,sequence:routeSequence}}));
    }));
  }else if(targetView==='global-search'){
    renderGlobalSearch(document.getElementById('globalSearchInput')?.value || '');
    updateGlobalSearchScopeButtons();
    renderGlobalDbLinks();
  }
...
function showHome(opts={}){
  const from=activePrimaryView();
  document.dispatchEvent(new CustomEvent('marketbase:before-viewchange',{detail:{from,to:'home'}}));
  const homeViewContent=document.getElementById('homeViewContent');
  const routeSequence=++primaryRouteSequence;
  const update=()=>{
    if(homeViewContent){homeViewContent.hidden=false;homeViewContent.removeAttribute('aria-hidden');}
    document.body.classList.remove('mb-view-active');
    document.querySelectorAll('.tab,.mb-primary-bottom-tab,.view').forEach(x=>x.classList.remove('active'));
    document.querySelector('.mb-primary-bottom-tab[data-home="true"]')?.classList.add('active');
    syncPrimaryViewChrome('');
    document.documentElement.classList.remove('mb-initial-subview');
    if(homeViewContent)markPrimaryRouteEnter(homeViewContent);
    if(Number.isFinite(Number(opts.restoreScroll))){
      window.requestAnimationFrame(()=>window.scrollTo({top:Math.max(0,Number(opts.restoreScroll)),left:0,behavior:'auto'}));
    }else if(opts.scroll!==false)window.requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}));
    document.dispatchEvent(new CustomEvent('marketbase:viewchange',{detail:{from,to:'home',sequence:routeSequence}}));
  };
  runPrimaryViewTransition(update,opts);
  if(opts.syncUrl!==false)syncPrimaryViewUrl('',{historyMode:opts.historyMode||'push'});
  return true;
}

function navigatePrimaryRoute(view,opts={}){
  const route=normalizedPrimaryRoute(view);
  if(!route)return showHome(opts);
  return switchView(route,opts);
}
window.MarketBaseInPageRouter=Object.freeze({
  navigate:navigatePrimaryRoute,
  showHome:opts=>showHome(opts||{}),
  active:activePrimaryView,
  fromLocation:primaryRouteFromLocation
});
window.addEventListener('popstate',event=>{
  const saved=Number(event.state?.mbScrollY);
  navigatePrimaryRoute(primaryRouteFromLocation(),{syncUrl:false,historyMode:'none',scroll:false,transition:true,restoreScroll:Number.isFinite(saved)?saved:0,source:'popstate'});
});
...
function fmtRiceValue(obj, metric){
  ...
  if(unit.includes('kg/人/年')) return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}kg／人／年`;
  if(unit.includes('kcal')) return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}kcal／人／日`;
  if(unit.includes('g/人/日')) return `${n.toLocaleString('ja-JP',{maximumFractionDigits:2})}g／人／日`;
  if(unit==='kg/ha') return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}kg／ha`;
  if(unit==='ha') return fmtJapaneseIntegerUnit(n,'ha');
  if(unit==='1000 USD') return `約${(n * RICE_VALUE_USD_TO_OKU_JPY).toLocaleString('ja-JP',{maximumFractionDigits:0})}億円`;
  if(unit==='%') return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}%`;
  return fmtJapaneseIntegerUnit(n,'t');
}
...
function fmtSchoolValue(obj, metric){
  ...
  if(unit==='%') return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}%`;
  if(unit==='点' || unit==='score') return `${n.toLocaleString('ja-JP',{maximumFractionDigits:1})}点`;
  if(unit==='校' || unit==='schools') return fmtJapaneseIntegerUnit(n,'校');
  if(unit==='日' || unit==='days') return `${Math.round(n).toLocaleString('ja-JP')}日`;
  return fmtJapaneseRankingPopulation(n);
}
...
function renderSchoolRanking(){
  ...
  list.innerHTML=viewItems.map(item=>{
    ...
    const name=item.name_ja || item.name_en || nameOf(e) || item.entity_id;
    return rankingRowHtml(item, e, name, fmtSchoolValue(item, metric), `${item.year || '—'} / ${item.source || 'WFP/GCNF/UNESCO'}`, barMax, item.rank<=3?'top-rank':'');
  }).join('');
}
...
function renderRiceRanking(){
  ...
  list.innerHTML=viewItems.map(item=>{
    const e=entities.find(x=>x.entity_id===item.entity_id) || item;
    const name=item.name_ja || item.name_en || nameOf(e) || item.entity_id;
    return rankingRowHtml(item, e, name, fmtRiceValue(item, metric), `${item.year || '—'} / ${item.source || 'FAOSTAT'}`, barMax, item.rank<=3?'top-rank':'');
  }).join('');
}
...
function formatJapanRankingValue(item, metric){
  ...
  if(metric==='japan_relation_summary'){
    ...
    return `${n.toLocaleString('ja-JP')}${level}`;
  }
  if(metric==='overseas_japanese_residents_total') return fmtJapaneseRankingPopulation(n);
  if(metric==='overseas_japanese_residents_mofa_region_totals') return fmtJapaneseRankingPopulation(n);
  if(metric==='overseas_japanese_long_term_residents') return fmtJapaneseRankingPopulation(n);
  if(metric==='overseas_japanese_permanent_residents') return fmtJapaneseRankingPopulation(n);
  if(metric==='overseas_japanese_residents_yoy_rate') return `${(n*100).toLocaleString('ja-JP',{maximumFractionDigits:1})}%`;
  if(metric==='japanese_restaurants_count') return fmtJapaneseIntegerUnit(n,'店');
  if(metric==='japanese_restaurants_app_region_totals') return fmtJapaneseIntegerUnit(n,'店');
  if(metric==='japanese_retail_foodservice_presence'){
    const brands=(item.brands||[]).filter(Boolean).join('／');
    return `${fmtJapaneseIntegerUnit(n,'件')}${brands ? `／${brands}` : ''}`;
  }
...
function renderJapanRanking(){
  ...
  list.innerHTML=items.map(item=>{
    const e=entities.find(x=>x.entity_id===item.entity_id) || item;
    const name=item.name_ja || nameOf(e) || item.entity_id;
    return rankingRowHtml(item, e, name, formatJapanRankingValue(item, metric), japanRankingSubline(item, metric), barMax, `japan-ranking-row ${item.rank<=3?'top-rank':''}`);
  }).join('');
}
...
const COUNTRY_INITIAL_RENDER_LIMIT=30;
const COUNTRY_RENDER_CHUNK_SIZE=24;
let countryRenderGeneration=0;
function countryCardHtml(e){
...
function scheduleCountryRenderChunk(callback){
...
function renderCountries(){
  const generation=++countryRenderGeneration;
  ...
  if(!list)return;
  if(!rows.length){list.removeAttribute('aria-busy');list.innerHTML=`<p class="notice">${t('noResults')}</p>`;return;}
  const initialEnd=Math.min(COUNTRY_INITIAL_RENDER_LIMIT,rows.length);
  list.innerHTML=rows.slice(0,initialEnd).map(countryCardHtml).join('');
  if(initialEnd>=rows.length){list.removeAttribute('aria-busy');return;}
  list.setAttribute('aria-busy','true');
  const progress=document.createElement('p');
  progress.className='notice mb-country-progress-status';
  progress.dataset.mbCountryProgress='';
  progress.setAttribute('role','status');
  progress.textContent=`国・地域を表示しています… ${initialEnd} / ${rows.length}`;
  list.append(progress);
  let cursor=initialEnd;
  const appendNext=()=>{
    if(generation!==countryRenderGeneration)return;
    if(document.visibilityState==='hidden'){
      window.setTimeout(()=>scheduleCountryRenderChunk(appendNext),300);
      return;
    }
    const end=Math.min(rows.length,cursor+COUNTRY_RENDER_CHUNK_SIZE);
    progress.insertAdjacentHTML('beforebegin',rows.slice(cursor,end).map(countryCardHtml).join(''));
    cursor=end;
    if(cursor<rows.length){
      progress.textContent=`国・地域を表示しています… ${cursor} / ${rows.length}`;
      scheduleCountryRenderChunk(appendNext);
      return;
    }
    progress.remove();
    list.removeAttribute('aria-busy');
    document.dispatchEvent(new CustomEvent('marketbase:countries-rendered',{detail:{count:rows.length,generation}}));
  };
  scheduleCountryRenderChunk(appendNext);
}
document.addEventListener('marketbase:viewchange',event=>{
  if(event.detail?.to==='countries')return;
  const list=document.getElementById('countryList');
  if(!list?.hasAttribute('aria-busy'))return;
  countryRenderGeneration+=1;
  list.removeAttribute('aria-busy');
  list.querySelector('[data-mb-country-progress]')?.remove();
  renderedPrimaryViews.delete('countries');
});
...
function renderRankings(){
  ...
  list.innerHTML=items.map(item=>{
    const e=entities.find(x=>x.entity_id===item.entity_id) || item;
    const name=lang==='en' ? (item.name_en || e.names?.en || item.entity_id) : (item.name_ja || e.names?.ja || nameOf(e) || item.entity_id);
    return rankingRowHtml(item, e, name, formatUnifiedRankingValue(item, sel.domain, sel.metric), unifiedRankingSourceLine(item, sel.domain, sel.metric), barMax, item.rank<=3?'top-rank':'');
  }).join('') + more;
}
...
function countryDistinctiveFactForEntity(entityId){
  return window.MARKET_BASE_COUNTRY_DISTINCTIVE_FACTS?.items?.[entityId] || null;
}

function countryDistinctiveFactHtml(e){
  const fact=countryDistinctiveFactForEntity(e.entity_id);
  if(!fact) return '';
  const source=(fact.source_url && /^https?:\/\//i.test(fact.source_url))
    ? `<a class="country-distinctive-fact-source" href="${safe(fact.source_url)}" target="_blank" rel="noopener noreferrer"><span>出典</span><strong>${safe(fact.source_name || '資料')} ↗</strong></a>`
    : '';
  return `<article class="country-distinctive-fact" aria-label="この国ならでは">
    <div class="country-distinctive-fact-heading"><span>この国ならでは</span><em>${safe(fact.category || '国別情報')}</em></div>
    <h4>${safe(fact.title || '')}</h4>
    <p data-thesis-highlight>${safe(fact.body || '')}</p>
    ${source}
  </article>`;
}
...
function countryProfilePowerHtml(profile){
  ...
  if(!p) return `<article class="country-profile-power is-missing">
    <div class="country-profile-power-heading"><div><span>電源情報</span><h4>電源・コンセント</h4></div><em>未確認</em></div>
    <div class="country-profile-power-layout">
      <div class="country-profile-plug-panel"><span class="country-profile-power-label">コンセント形状</span><div class="country-profile-power-missing">確認できず</div></div>
      <div class="country-profile-power-values">
        <div><span>家庭用電源</span><strong>確認できず</strong></div>
        <div><span>産業用機械</span><strong>確認できず</strong></div>
      </div>
    </div>
    <div class="country-profile-power-note"><strong>設備導入時の注意</strong><p>電圧・周波数・相数・接地方式は、設置先と現地の電力会社へ確認してください。</p></div>
  </article>`;
...
function countryBasicStatsHtml(e){
  ...
  return `<section class="detail-section country-profile-basic-stats" id="country-basic-stats"><h3>概要・基本統計</h3><div class="stats-grid">${incomeCard}${UI_METRICS.map(m=>{
    const s=getStat(e,m); const missing=isMissing(s);
...
function countryProfileBlock(e){
  ...
  return `${countryProfileTransportHtml(profile)}<section class="detail-section country-profile-section">
    ...
    ${countryDistinctiveFactHtml(e)}
    ${countryBasicStatsHtml(e)}
    ${wdiMetadataBlock(e)}
...
function detailContentHtml(e){
  const profileHtml=countryProfileBlock(e);
  const profileFallback=profileHtml ? '' : `${countryBasicStatsHtml(e)}${wdiMetadataBlock(e)}`;
  return `<div class="detail-header"><div class="detail-flag">${flagMarkup(e, 'flag-img-detail')}</div><div class="detail-title-block"><h2>${safe(nameOf(e))}</h2><p class="country-sub">${safe(e.entity_id)} / ${safe(e.iso3)} / ${safe(displayRegion(e))} / ${safe(displaySubregion(e))}</p><div class="detail-mini-tabs"><span>国別情報</span><span>基本統計</span><span>市場</span><span>米</span><span>学校給食</span><span>日本関連</span><span>出典</span></div></div></div>${profileHtml}${profileFallback}${countryDatabaseLinksHtml(e)}${countryNewsHostHtml(e)}<section class="detail-section">${marketSummaryBlock(e)}</section>`;
}
...
function renderDetailContent(e){
  ...
  ensureOptionalDataForView('detail',{reason:'country-detail'});
  document.dispatchEvent(new CustomEvent('marketbase:prepare-country-detail',{detail:{entityId:e?.entity_id||''}}));
  const ensure=window.MarketBaseHomeDeferred?.ensureCountryDetail?.()||Promise.resolve(true);
  return Promise.resolve(ensure).catch(error=>{
    console.warn('MARKET BASE country detail extras could not be loaded; basic data will be shown.',error);
    return false;
  }).then(()=>renderLoadedDetailContent(e,sequence));
}
function renderLoadedDetailContent(e,sequence){
  const root=document.getElementById('detailContent');
  if(!root) return Promise.resolve(false);
  if(sequence!==detailRenderSequence||currentDetailEntityId!==e.entity_id)return Promise.resolve(false);
...
function openDetail(id){
  ...
  document.dispatchEvent(new CustomEvent('marketbase:prepare-country-detail',{detail:{entityId:id}}));
...
function initControls(){
  ...
  refreshRankingMetricOptions({reset:true});
...
const todaysJourneySessionBaseDate=(()=>{
  ...
  if(/^\d{2}-\d{2}$/.test(String(override||''))){
    ...
    const candidate=new Date(base.getFullYear(),month-1,day,12,0,0,0);
...
function todaysJourneyDateKey(offset=todaysJourneyOffset){
  ...
  const target=todaysJourneyBaseDate();
  target.setDate(target.getDate()-days);
  let key=`${String(target.getMonth()+1).padStart(2,'0')}-${String(target.getDate()).padStart(2,'0')}`;
  if(key==='02-29') key='02-28';
  return key;
}
function todaysJourneyRelativeLabel(offset=todaysJourneyOffset){
  const days=Math.max(0,Math.min(TODAYS_JOURNEY_MAX_HISTORY_DAYS,Number(offset)||0));
  if(days===0) return '今日';
  if(days===1) return '昨日';
  if(days===2) return '一昨日';
  return `${days}日前`;
}
function updateTodaysJourneyHistoryControls(){
  const previous=document.getElementById('todaysJourneyPrev');
  const next=document.getElementById('todaysJourneyNext');
  const relative=document.getElementById('todaysJourneyRelative');
  const current=todaysJourneyRelativeLabel();
  if(relative) relative.textContent=current;
  if(previous){
    const targetLabel=todaysJourneyRelativeLabel(Math.min(TODAYS_JOURNEY_MAX_HISTORY_DAYS,todaysJourneyOffset+1));
    previous.disabled=todaysJourneyOffset>=TODAYS_JOURNEY_MAX_HISTORY_DAYS;
    previous.setAttribute('aria-label',previous.disabled?'7日前より古い記事は表示できません':`${targetLabel}の記事を見る`);
    previous.title=previous.getAttribute('aria-label')||'';
  }
  if(next){
    const targetLabel=todaysJourneyRelativeLabel(Math.max(0,todaysJourneyOffset-1));
    next.disabled=todaysJourneyOffset<=0;
    next.setAttribute('aria-label',next.disabled?'今日より新しい記事はありません':`${targetLabel}の記事を見る`);
    next.title=next.getAttribute('aria-label')||'';
  }
}
function setTodaysJourneyOffset(value){
  const next=Math.max(0,Math.min(TODAYS_JOURNEY_MAX_HISTORY_DAYS,Number(value)||0));
  if(next===todaysJourneyOffset){ updateTodaysJourneyHistoryControls(); return; }
  todaysJourneyOffset=next;
  renderTodaysJourney();
}
function initTodaysJourneyHistory(){
  const queryOffset=Number(new URLSearchParams(window.location.search).get('journey_offset'));
  if(Number.isInteger(queryOffset)&&queryOffset>=0&&queryOffset<=TODAYS_JOURNEY_MAX_HISTORY_DAYS) todaysJourneyOffset=queryOffset;
  document.getElementById('todaysJourneyPrev')?.addEventListener('click',()=>setTodaysJourneyOffset(todaysJourneyOffset+1));
  document.getElementById('todaysJourneyNext')?.addEventListener('click',()=>setTodaysJourneyOffset(todaysJourneyOffset-1));
  updateTodaysJourneyHistoryControls();
}
...
function writeJourneyImageCache(entry,image){
  if(!image?.image_url||image.is_fallback) return;
  try{ localStorage.setItem(journeyImageCacheKey(entry),JSON.stringify({saved_at:Date.now(),image})); }catch(_){}
}
function removeJourneyImageCache(entry){
  try{ localStorage.removeItem(journeyImageCacheKey(entry)); }catch(_){}
}
function failedJourneyImageUrls(entry){
  return journeyImageFailures.get(entry?.id)||new Set();
}
function markJourneyImageFailed(entry,url){
  if(!entry?.id||!url) return;
  const failed=failedJourneyImageUrls(entry);
  failed.add(String(url));
  while(failed.size>16) failed.delete(failed.values().next().value);
  journeyImageFailures.set(entry.id,failed);
  const current=journeyImageMemory.get(entry.id);
  if(current?.image_url===url) journeyImageMemory.delete(entry.id);
  const cached=readJourneyImageCache(entry);
  if(cached?.image_url===url) removeJourneyImageCache(entry);
}
function isJourneyImageUrl(url){
  const value=String(url||'').trim();
  return /^(?:https?:|data:image\/|assets\/|\.\/assets\/)/i.test(value);
}
function escapeJourneySvgText(value){
...
async function fetchJourneyJson(url,timeoutMs=6500){
  ...
  try{
    ...
    if(!response.ok) throw new Error(`journey image API ${response.status}`);
...
async function searchJourneyImageCandidates(entry,exclude=new Set()){
  ...
  for(const row of ranked){
    const image=journeyImageFromPage(row.page);
    const url=String(image?.image_url||'');
    if(!url||urls.has(url)||exclude.has(url)) continue;
    urls.add(url); images.push(image);
...
async function resolveTodaysJourneyImageNow(entry,options={}){
  ...
  if(image){
    journeyImageMemory.set(entry.id,image);
    writeJourneyImageCache(entry,image);
    return image;
  }
...
function applyTodaysJourneyImage(entry,image,renderToken){
  ...
  thumb.onerror=async()=>{
    ...
    thumb.onerror=null;
    const retry=await resolveTodaysJourneyImage(entry,{forceSearch:true,exclude:[image.image_url]});
    if(renderToken!==todaysJourneyRenderToken) return;
...
async function renderTodaysJourney(){
  const renderToken=++todaysJourneyRenderToken;
  ...
  const relativeLabel=todaysJourneyRelativeLabel();
  ...
  const imageBtn=document.getElementById('todaysJourneyImageBtn');
  updateTodaysJourneyHistoryControls();
  root.dataset.journeyOffset=String(todaysJourneyOffset);
  root.setAttribute('aria-label',`Today’s Journey ${relativeLabel}`);
  ...
  card?.classList.remove('has-photo','photo-loading');
  if(photo){ photo.hidden=true; photo.onclick=null; photo.setAttribute('aria-label',`${relativeLabel}の写真を拡大表示`); }
  if(imageBtn){ imageBtn.hidden=true; imageBtn.onclick=null; }
  if(thumb){ thumb.removeAttribute('src'); thumb.alt=''; thumb.onerror=null; }
  if(countryBtn){ countryBtn.hidden=true; countryBtn.onclick=null; }
  ...
  if(image&&photo&&thumb&&imageBtn){
    applyTodaysJourneyImage(entry,image,renderToken);
  }else if(photo&&thumb){
    applyTodaysJourneyImage(entry,journeyFallbackImage(entry),renderToken);
  }
}
...
function renderAll(){
  applyI18n(); initControls(); renderMetricOverview(); renderKnownGaps(); renderRecentEntities(); renderCompanyHomeSearch(); renderGlobalDbLinks(); renderTodaysJourney();
  document.getElementById('entityCount').textContent=entities.length;
}
function applyMarketBaseDataset(bundle){
  renderedPrimaryViews.clear();
  // The embedded fallback already carries the optional payloads. Remember
  // those keys so a later route does not download a second copy after a core
  // network failure (file:// mode remains fully self-contained as well).
  Object.keys(OPTIONAL_DATA_SPECS).forEach(key=>{if(bundle[key])optionalDataReady.add(key);});
  entities=bundle.entities?.entities||[];
  rankings=bundle.rankings?.rankings||{};
  retailData=null; retailPresenceData=null; retailProfilesData=null;
  retailCountryCardSummaryData=bundle.retail_country_card_summary||window.MB_RETAIL_COUNTRY_CARD_SUMMARY_DATA||null;
  // A direct deep-link can request its optional group while the three core
  // files are still in flight. Keep any group that arrived first instead of
  // clearing it when the core bundle is applied.
  riceData=bundle.rice_data||riceData;
  riceRankings=bundle.rice_rankings?.rankings||riceRankings;
  schoolMealsData=bundle.school_meals_data||schoolMealsData;
  schoolMealRankings=bundle.school_meals_rankings?.rankings||schoolMealRankings;
  priority4ReadyData=bundle.priority4_ready||priority4ReadyData;
  wdiCountryMetadata=bundle.wdi_country_metadata||wdiCountryMetadata;
  japanRelatedData=bundle.japan_related_data||japanRelatedData;
  japanRelatedRankings=bundle.japan_related_rankings?.rankings||japanRelatedRankings;
  japaneseRestaurantsOverview=bundle.japanese_restaurants_overview||japaneseRestaurantsOverview;
  overseasJapaneseResidentsOverview=bundle.overseas_japanese_residents_overview||overseasJapaneseResidentsOverview;
  renderAll();
  const activeView=activePrimaryView();
  if(activeView==='compare'){
    renderPrimaryViewOnDemand('rankings');
    renderCompare();
  }else if(activeView){
    renderPrimaryViewOnDemand(activeView);
  }
  openRequestedCountryFromUrl();
}
function applyOptionalMarketBaseDataset(bundle){
  riceData=bundle.rice_data||riceData;
  riceRankings=bundle.rice_rankings?.rankings||riceRankings;
  schoolMealsData=bundle.school_meals_data||schoolMealsData;
  schoolMealRankings=bundle.school_meals_rankings?.rankings||schoolMealRankings;
  ...
  japaneseRestaurantsOverview=bundle.japanese_restaurants_overview||japaneseRestaurantsOverview;
  overseasJapaneseResidentsOverview=bundle.overseas_japanese_residents_overview||overseasJapaneseResidentsOverview;
  refreshRankingMetricOptions();
  ['rankings','rice','school','japan'].forEach(view=>renderedPrimaryViews.delete(view));
  const activeView=activePrimaryView();
  if(activeView==='compare'){
    renderPrimaryViewOnDemand('rankings');
    renderCompare();
  }else if(['rankings','rice','school','japan'].includes(activeView)){
    renderPrimaryViewOnDemand(activeView);
  }
  const detailDialog=document.getElementById('detailDialog');
  const detailEntity=entities.find(entity=>entity.entity_id===currentDetailEntityId);
  if(detailDialog?.open&&detailEntity) renderDetailContent(detailEntity).catch(()=>{});
}
async function loadMarketBaseEmbeddedData(){
  if(window.MARKET_BASE_EMBEDDED_DATA) return window.MARKET_BASE_EMBEDDED_DATA;
  if(window.MarketBaseRuntime?.loadScript){
    await window.MarketBaseRuntime.loadScript('embedded-data.js','MARKET_BASE_EMBEDDED_DATA');
  }else{
    await new Promise((resolve,reject)=>{
      const script=document.createElement('script');
      script.src='embedded-data.js?v=20260725-r11348';
      script.onload=resolve; script.onerror=()=>reject(new Error('embedded-data.js load failed'));
      document.head.append(script);
    });
  }
  if(!window.MARKET_BASE_EMBEDDED_DATA) throw new Error('embedded dataset is unavailable');
  return window.MARKET_BASE_EMBEDDED_DATA;
}
async function fetchMarketBaseJson(path){
  const scoped=window.MarketBaseRuntime?.assetUrl?window.MarketBaseRuntime.assetUrl(path):`${path}${String(path).includes('?')?'&':'?'}v=${encodeURIComponent(window.MARKET_BASE_BUILD?.assetVersion||'20260725-r11348')}`;
  const res=await fetch(scoped,{cache:'default'});
  if(!res.ok) throw new Error(`${path} ${res.status}`);
  return res.json();
}
const OPTIONAL_DATA_SPECS=Object.freeze({
  rice_data:'data/market_base_rice_data_CURRENT.json',
  rice_rankings:'data/market_base_rice_rankings_CURRENT.json',
  school_meals_data:'data/market_base_school_meals_data_CURRENT.json',
  school_meals_rankings:'data/market_base_school_meals_rankings_CURRENT.json',
  priority4_ready:'data/market_base_priority4_ready_staging_v92.json',
  wdi_country_metadata:'data/market_base_wdi_country_metadata_v1.json',
  japan_related_data:'data/market_base_japan_related_data_v1.json',
  japan_related_rankings:'data/market_base_japan_related_rankings_v1.json',
  japanese_restaurants_overview:'data/market_base_japanese_restaurants_overview_v1.json',
  overseas_japanese_residents_overview:'data/market_base_overseas_japanese_residents_overview_v1.json'
});
const OPTIONAL_DATA_GROUPS=Object.freeze({
  rice:['rice_data','rice_rankings'],
  school:['school_meals_data','school_meals_rankings'],
  japan:['japan_related_data','japan_related_rankings','japanese_restaurants_overview','overseas_japanese_residents_overview'],
  rankings:['rice_rankings','school_meals_rankings','japan_related_rankings'],
  compare:['rice_data','school_meals_data','japan_related_data','rice_rankings','school_meals_rankings','japan_related_rankings'],
  detail:['wdi_country_metadata','rice_data','school_meals_data','japan_related_data','japanese_restaurants_overview','overseas_japanese_residents_overview'],
  // This staging file currently has no visible consumer. Keep it addressable
  // by an explicit first-use intent without adding its 0.9 MB to any route.
  staging:['priority4_ready']
});
const optionalDataReady=new Set();
const optionalDataPromises=new Map();
function ensureOptionalDataKey(key){
  const path=OPTIONAL_DATA_SPECS[key];
  if(!path)return Promise.resolve(false);
  if(optionalDataReady.has(key))return Promise.resolve(true);
  if(optionalDataPromises.has(key))return optionalDataPromises.get(key);
  if(window.location.protocol==='file:'||navigator.onLine===false)return Promise.resolve(false);
  const request=fetchMarketBaseJson(path).then(value=>{
    optionalDataReady.add(key);
    applyOptionalMarketBaseDataset({[key]:value});
    document.dispatchEvent(new CustomEvent('marketbase:optional-data-ready',{detail:{key,path}}));
    return true;
  }).catch(error=>{
    console.warn(`MARKET BASE optional dataset delayed: ${path}`,error);
    document.dispatchEvent(new CustomEvent('marketbase:optional-data-error',{detail:{key,path,error}}));
    return false;
  }).finally(()=>optionalDataPromises.delete(key));
  optionalDataPromises.set(key,request);
  return request;
}
function ensureOptionalDataGroup(group){
  const keys=OPTIONAL_DATA_GROUPS[group]||[];
  if(!keys.length)return Promise.resolve(false);
  return Promise.all(keys.map(ensureOptionalDataKey)).then(results=>results.some(Boolean));
}
function optionalGroupsForView(view){
  const normalized=String(view||'').toLowerCase();
  return Object.prototype.hasOwnProperty.call(OPTIONAL_DATA_GROUPS,normalized)?[normalized]:[];
}
function ensureOptionalDataForView(view,options={}){
  const groups=optionalGroupsForView(view);
  if(!groups.length)return Promise.resolve(false);
  // Route/detail calls are explicit user intent. There is deliberately no
  // idle or boot warm-up: background, offline, data-saver, slow-network and
  // radio-playing sessions therefore receive no speculative optional fetch.
  if(document.visibilityState==='hidden'||navigator.onLine===false)return Promise.resolve(false);
  return Promise.all(groups.map(ensureOptionalDataGroup)).then(results=>results.some(Boolean));
}
function resumeOptionalDataForActiveSurface(){
  if(document.visibilityState==='hidden'||navigator.onLine===false)return;
  const detailDialog=document.getElementById('detailDialog');
  if(detailDialog?.open){ensureOptionalDataForView('detail',{reason:'resume'});return;}
  const active=activePrimaryView();
  if(active)ensureOptionalDataForView(active,{reason:'resume'});
}
window.addEventListener('online',resumeOptionalDataForActiveSurface);
document.addEventListener('visibilitychange',resumeOptionalDataForActiveSurface);
document.addEventListener('marketbase:optional-data-intent',event=>{
  const group=String(event.detail?.group||'');
  if(group)ensureOptionalDataGroup(group);
});
function showMarketBaseLoadError(err){
  const main=document.querySelector('main');
  if(!main) return;
  main.insertAdjacentHTML('afterbegin',`<div class="error-box">データを読み込めませんでした。ファイル一式が揃っているか確認してください。<br>${safe(err?.message||err)}</div>`);
}
async function boot(){
  const fileMode=window.location.protocol==='file:';
  if(fileMode){
    try{ applyMarketBaseDataset(await loadMarketBaseEmbeddedData()); }
    catch(err){ showMarketBaseLoadError(err); }
    return;
  }
  try{
    const coreResult=Promise.all([
      fetchMarketBaseJson('data/market_base_entities_basic_stats_full196_rc.json'),
      fetchMarketBaseJson('data/market_base_rankings_basic_stats_full196_rc.json'),
      fetchMarketBaseJson('data/market_base_retail_country_card_summary_CURRENT.json')
    ]);
    const [e,r,retailCountrySummary]=await coreResult;
    applyMarketBaseDataset({
      entities:e,rankings:r,retail_country_card_summary:retailCountrySummary
    });
  }catch(networkError){
    console.warn('MARKET BASE current JSON load failed; synchronized embedded dataset will be used.',networkError);
    try{ applyMarketBaseDataset(await loadMarketBaseEmbeddedData()); }
    catch(embeddedError){ showMarketBaseLoadError(embeddedError); }
  }
}
...
function openCountriesWithCurrentFilters(){
  // Let switchView paint the selected/busy state before it creates even the
  // first 30 SVG-bearing cards. Home presets therefore do no list work in the
  // original tap task.
  countryRenderGeneration+=1;
  renderedPrimaryViews.delete('countries');
...
function handleAppNavigationClick(btn, event){
  ...
  if(btn.dataset.openEntity){
    ...
    if(!openEntityByAnyCode(btn.dataset.openEntity)){
      ...
      openCountriesWithCurrentFilters();
    }
    return true;
  }
...
function refreshMarketBaseCache(event){
  if(window.MarketBaseUpdate?.refresh) return window.MarketBaseUpdate.refresh({event,broadcast:true});
  if(event){ event.preventDefault(); event.stopPropagation(); }
  // A plain reload is the only safe fallback when the canonical controller
  // failed to load. Never unregister workers or delete shared CacheStorage.
  window.location.reload();
  return false;
}
window.refreshMarketBaseCache=refreshMarketBaseCache;
...
document.querySelectorAll('.mb-primary-bottom-tab[data-view]').forEach(btn=>btn.addEventListener('click',()=>switchView(btn.dataset.view)));
document.querySelectorAll('.mb-primary-bottom-tab[data-home]').forEach(btn=>btn.addEventListener('click',()=>showHome()));
document.querySelectorAll('[data-ranking-compare-tab]').forEach(btn=>{
  btn.addEventListener('click',()=>setRankingCompareTab(btn.dataset.rankingCompareTab));
  btn.addEventListener('keydown',event=>{
    if(!['ArrowLeft','ArrowRight'].includes(event.key)) return;
    event.preventDefault();
    setRankingCompareTab(btn.dataset.rankingCompareTab==='rankings'?'compare':'rankings',{focus:true});
  });
});
...
document.addEventListener('marketbase:deferred-ready',event=>{
  if(event.detail?.group==='journey')renderTodaysJourney().catch(()=>{});
  if(event.detail?.group==='country'){
    const dialog=document.getElementById('detailDialog');
    const entity=entities.find(item=>item.entity_id===currentDetailEntityId);
    if(dialog?.open&&entity)renderDetailContent(entity).catch(()=>{});
  }
});
document.querySelectorAll('[data-preset]').forEach(btn=>btn.addEventListener('click',()=>applyPreset(btn.dataset.preset)));
document.querySelectorAll('[data-search-preset]').forEach(btn=>btn.addEventListener('click',()=>{document.getElementById('searchInput').value=btn.dataset.searchPreset; document.getElementById('gapOnlyToggle').checked=false; openCountriesWithCurrentFilters();}));
document.querySelector('[data-clear-search]')?.addEventListener('click',()=>{document.getElementById('searchInput').value=''; document.getElementById('regionFilter').value='all'; setSubregionPreset('all'); updateSubregionOptions();
  const incomeFilter=document.getElementById('incomeFilter'); if(incomeFilter) incomeFilter.value='all'; document.getElementById('gapOnlyToggle').checked=false; openCountriesWithCurrentFilters();});
document.querySelectorAll('[data-open-entity]').forEach(btn=>btn.addEventListener('click',()=>{ const id=btn.dataset.openEntity; const e=entities.find(x=>x.entity_id===id || x.iso2===id || x.iso3===id); if(e){ openDetail(e.entity_id); } else { document.getElementById('searchInput').value=id; openCountriesWithCurrentFilters(); }}));
...
(async function initializeMarketBase(){
  // The canonical update controller owns version checks. Waiting for the
  // legacy probe here caused two network checks and blocked the first render.
  await boot();
})().catch(showMarketBaseLoadError);


// R61: primary screen swipe navigation.
// Order: Home -> Currency -> Learn -> Rankings. Ranking and Compare share one page.
...
(function initPrimarySwipeNavigation(){
  const BLOCKED_SELECTOR=[
    ...
    '.mb-primary-bottom-nav','.tabs','.ranking-compare-tabs','.toolbar','.compare-selects','.compare-table',
  ...
  function currentPrimary(){
    const active=document.querySelector('.view.active')?.id || '';
    if(document.body.classList.contains('mb-view-active') && (active==='learn'||active==='rankings')) return active;
  ...
  function navigate(direction){
    ...
    if(current==='learn' && direction==='previous'){
      showSwipeHint('通貨換算','previous');
      window.setTimeout(()=>window.location.assign('market-base-currency-converter-v273-r29.html?from=learn-swipe'),100);
      return true;
    }
    if(current==='learn' && direction==='next'){
      showSwipeHint('ランキング','next');
      switchView('rankings');
      return true;
    }
    if(current==='rankings' && direction==='previous'){
      showSwipeHint('学ぶ','previous');
      switchView('learn');
      return true;
    }
    return false;
...
const requestedView=normalizedPrimaryRoute(pageParams.get('view'));
try{
  const initialState=(history.state&&typeof history.state==='object')?history.state:{};
  history.replaceState({...initialState,mbRoute:true,mbView:requestedView,mbScrollY:primaryRouteScrollState()},'',location.href);
}catch(_){}
if(requestedView){
  window.setTimeout(()=>navigatePrimaryRoute(requestedView,{scroll:false,syncUrl:false,historyMode:'none',transition:false,source:'initial-url'}),0);
}else{
  document.documentElement.classList.remove('mb-initial-subview');
}
...
if(requestedCountry){
  ...
  if(returnState?.view && document.getElementById(returnState.view)){
    window.setTimeout(()=>switchView(returnState.view,{scroll:false,historyMode:'replace'}),0);
  }
