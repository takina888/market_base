(function(){
  ...
  function goHome(event){
    if(!isMain)return;
    const router=window.MarketBaseInPageRouter;
    if(!router||typeof router.showHome!=='function')return;
    event.preventDefault();
    event.stopImmediatePropagation();
    router.showHome({source:'unified-shell'});
  }

  document.addEventListener('click',function(event){
    const target=event.target.closest('button,a');
    if(!target)return;
    if(isMain&&(target.matches('[data-view-back-home]')||target.matches('.mb-primary-bottom-tab[data-home], [data-home="true"]')))goHome(event);
  },true);
