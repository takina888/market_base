(function (global) {
  ...
  const SERVICE_WORKER_TIMEOUT_MS = 5500;
  ...
  function loadRadioDock() {
    if (isRadioPlayerPage() || isStandaloneCodePage()) return;
  ...
  async function fetchRemoteVersion() {
    ...
    const timeout = global.setTimeout(() => abortController?.abort(), 3500);
    try {
  ...
  async function performRefresh(remoteVersion, options) {
    ...
    setWorking(true);
    try {
      // Service-worker activation is useful but must never leave the button
      // stuck or block a cache-busting reload on iPhone PWA.
      await withTimeout(
        ...
        SERVICE_WORKER_TIMEOUT_MS + 500,
        'service worker soft timeout'
      ).catch(() => {});
      await withTimeout(pruneOldSiteCaches(remoteVersion), 2500, 'cache cleanup timed out').catch(() => {});
