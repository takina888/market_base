(function (global) {
  ...
  const PLAYER_WINDOW_NAME = 'marketBaseWorldRadioPlayer';
  ...
  let collapsed = collapsedPreference !== '0';
  let dismissed = false;
      ...
      <section class="mb-radio-dock-panel" id="mbRadioDockPanel" aria-label="世界のラジオ操作">
        <div class="mb-radio-dock-head">
          ...
          <div class="mb-radio-dock-copy">
            ...
            <span class="mb-radio-dock-track" id="mbRadioDockTrack" hidden>
              <span class="mb-radio-dock-track-row"><em>曲名</em><b id="mbRadioDockTrackTitle">—</b></span>
              <span class="mb-radio-dock-track-row"><em>歌手</em><small id="mbRadioDockTrackArtist">—</small></span>
            </span>
        ...
        <div class="mb-radio-dock-actions">
          ...
          <a class="mb-radio-dock-open" id="mbRadioDockOpen"
            href="${new URL('world-radio/player.html?id=wnyc&autoplay=1&v=20260803-v333-1-bottom-tool-menu-radio-currency-safe', siteRoot).href}"
            target="marketBaseWorldRadioPlayer"
  ...
  function positionBounds() {
    ...
    const maximumX = Math.max(
      minimumX,
      global.innerWidth - safeRight - (global.innerWidth >= 900 ? 18 : 0) - panelWidth
    );
  ...
  function renderTrack(state) {
    ...
    const active = !!(
      ...
      ['playing', 'loading', 'interrupted'].includes(state.status)
    );
    let displayTitle = title;
    let displayArtist = artist || state.stationName || '';

    if (!displayTitle) {
      if (active && state.metadataExpected) {
        displayTitle = '曲情報を取得中…';
        displayArtist = '放送局から曲名・アーティスト情報を待っています';
      } else if (active) {
        displayTitle = '曲名・アーティスト情報なし';
        displayArtist = 'この放送局は曲情報を配信していません';
      } else {
        displayTitle = '曲名・アーティスト';
        displayArtist = '再生すると、提供される局ではここに表示します';
      }
    }

    trackBox.hidden = false;
    placeLabel.hidden = true;
    trackTitleLabel.textContent = displayTitle;
    trackArtistLabel.textContent = displayArtist;
    trackTitleLabel.title = displayTitle;
    trackArtistLabel.title = displayArtist;
  }
  ...
  function render(state = readState()) {
    ...
    if (!global.navigator.onLine) {
      ...
      dock.dataset.playing = 'false';
      dock.dataset.state = 'offline';
    ...
    const radioActive = !!(currentState && (
      currentState.active || currentState.playing ||
      ['playing', 'loading', 'interrupted'].includes(currentState.status)
    ));
    dock.dataset.active = String(radioActive);
    ...
    if (!currentState) {
      ...
      openLink.href = new URL(
        'world-radio/player.html?id=wnyc&autoplay=1&v=20260803-v333-1-bottom-tool-menu-radio-currency-safe',
        siteRoot
      ).href;
      message.textContent = '';
    ...
    openLink.href = new URL(
      `world-radio/player.html?id=${encodeURIComponent(currentState.stationId)}&autoplay=1&v=20260803-v333-1-bottom-tool-menu-radio-currency-safe`,
      siteRoot
    ).href;
  ...
  function openPlayerWindow(event) {
    if (event) event.preventDefault();
    if (!global.navigator.onLine) {
      temporaryMessage('オンライン時に利用できます。');
      return;
    }
    const opened = global.open(openLink.href, PLAYER_WINDOW_NAME);
    try { opened?.focus?.(); } catch (_) {}
    if (!opened) global.location.href = openLink.href;
  }
  openLink.addEventListener('click', openPlayerWindow);
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) { render(); schedulePositionRestore(); }
  });
  global.addEventListener('pageshow', () => { render(); schedulePositionRestore(); });

  applyDismissed(dismissed, false);
  render();
  global.requestAnimationFrame(() => { delete dock.dataset.initializing; });
  ...
  global.MarketBaseRadioDock = Object.freeze({
    ...
    hide: () => { collapsedPreference = '1'; storageSet(COLLAPSED_KEY, collapsedPreference); applyCollapsed(true); },
    isHidden: () => dismissed
  });
})(window);
