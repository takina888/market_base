(function (global) {
    ...
    tools: [
      {
        ...
        label: '計算機・単位換算',
        icon: 'calculator',
        href: 'market-base-currency-converter-v273-r29.html?tool=calculator&v=20260803-v333-4-radio-state-pc-shell-fix'
      },
      {
        id: 'currency',
        order: 20,
        label: '為替換算',
        icon: 'currency',
        href: 'market-base-currency-converter-v273-r29.html?tool=currency&v=20260803-v333-4-radio-state-pc-shell-fix'
      },
      {
        id: 'code',
        order: 900,
        label: 'WORK CODE',
        icon: 'code',
        href: 'market-base-code-tool.html?v=20260803-v333-4-radio-state-pc-shell-fix'
      }
    ]
  });
  ...
  let radioFinder = null;
  let collapsed = storageGet(COLLAPSED_KEY) === '1';
  ...
  function createDock() {
    ...
    node.innerHTML = `
      <section class="mb-radio-dock-panel mb-tool-dock-panel" id="mbSecondaryDockPanel"
        ...
        <div class="mb-tool-picker-row">
          <button class="mb-radio-dock-drag" id="mbSecondaryDockDrag" type="button"
            aria-label="ツール小窓を移動"><span aria-hidden="true">⠿</span></button>
          <div class="mb-tool-picker-shell">
            <button class="mb-tool-picker-trigger" id="mbToolPickerTrigger" type="button"
              aria-expanded="false" aria-controls="mbSecondaryDockLinks">
              <span>ツールを選択</span><b aria-hidden="true">⌄</b>
            </button>
            <div class="mb-secondary-dock-links" id="mbSecondaryDockLinks"
              role="menu" aria-label="ツール一覧" hidden></div>
          </div>
  ...
  function renderLinks() {
    ...
    config.tools.forEach(item => {
      ...
      link.innerHTML = `<span>${escapeHtml(item.label)}</span><b aria-hidden="true">›</b>`;
      link.addEventListener('click', () => {
        rememberTool(item.id, true);
        setPickerOpen(false);
      });
  ...
  function setPickerOpen(next) {
    pickerOpen = !!next;
    if (!pickerList || !pickerTrigger) return;
    pickerList.hidden = !pickerOpen;
    pickerTrigger.setAttribute('aria-expanded', String(pickerOpen));
    pickerTrigger.querySelector('b').textContent = pickerOpen ? '⌃' : '⌄';
    dock.dataset.pickerOpen = String(pickerOpen);
    scheduleReflow();
  }
  ...
  function attachEvents() {
    ...
    document.addEventListener('click', event => {
      if (!pickerOpen || dock.contains(event.target)) return;
      setPickerOpen(false);
    });
    ...
    global.addEventListener('storage', event => {
      if (event.key === COLLAPSED_KEY) applyCollapsed(event.newValue === '1', false);
  ...
  function attachRadioDock() {
    const found = document.querySelector('#marketBaseRadioDock');
    if (!found || found === radioDock) return !!radioDock;
    radioDock = found;
    observeRadio();
    scheduleReflow();
    return true;
  ...
  function watchForRadio() {
    attachRadioDock();
    if (radioDock || radioFinder) return;
    radioFinder = new MutationObserver(() => {
      if (!attachRadioDock()) return;
      radioFinder.disconnect();
      radioFinder = null;
    });
    radioFinder.observe(document.documentElement, { childList: true, subtree: true });
  }
  ...
  function init() {
    ...
    pickerTrigger = dock.querySelector('#mbToolPickerTrigger');
    pickerList = dock.querySelector('#mbSecondaryDockLinks');
    renderLinks();
    syncToolLinks();
