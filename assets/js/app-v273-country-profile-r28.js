...
function countryProfileTransportHtml(profile){
  ...
  if(!rows) return '';
  return `<article class="country-profile-transport"><h4>主要都市の地下鉄・鉄道路線図</h4><p>交通事業者などが公開する路線図を直接開きます。発行年のある地図はリンク名に記載しています。</p><div class="country-profile-transport-list">${rows}</div></article>`;
}
...
function countryProfileBlock(e){
  ...
  return `${countryProfileTransportHtml(profile)}<section class="detail-section country-profile-section">
    <div class="country-profile-heading"><div><span>COUNTRY PROFILE</span><h3>この国・地域について</h3></div><em>国別情報</em></div>
...
function detailContentHtml(e){
  ...
  return `<div class="detail-header"><div class="detail-flag">${flagMarkup(e, 'flag-img-detail')}</div><div class="detail-title-block"><h2>${safe(nameOf(e))}</h2><p class="country-sub">${safe(e.entity_id)} / ${safe(e.iso3)} / ${safe(e.region)} / ${safe(e.subregion)}</p><div class="detail-mini-tabs"><span>国別情報</span><span>基本統計</span><span>市場</span><span>米</span><span>学校給食</span><span>日本関連</span><span>出典</span></div></div></div>${countryProfileBlock(e)}${statsHtml}${wdiMetadataBlock(e)}<section class="detail-section">${marketSummaryBlock(e)}</section>`;
}
...
const MARKET_BASE_APP_VERSION = 'V273_EIGHT_SPECIALIST_DB_R32U8_20260716';
...
if(['countries','compare','rankings'].includes(requestedView)){
  window.setTimeout(()=>switchView(requestedView,{scroll:false}),0);
}
