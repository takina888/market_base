(function(){
  ...
  var UPDATE_CONTROLLER_URL=new URL('assets/js/market-base-update-controller-v335.js?v=20260907-v333-19-jfm-v077-currency-hf2',SITE_ROOT);
  var NAVIGATION_URL=new URL('assets/js/market-base-navigation-v333-18.js?v=20260907-v333-19-jfm-v077-currency-hf2',SITE_ROOT);
  var NAVIGATION_STYLE_URL=new URL('assets/css/market-base-navigation-v333-18.css?v=20260907-v333-19-jfm-v077-currency-hf2',SITE_ROOT);
  ...
  function ensureNavigationStyles(){
    ...
    var link=document.createElement('link');link.rel='stylesheet';link.href=NAVIGATION_STYLE_URL.href;link.dataset.mbNavigationStyle='';document.head.appendChild(link);
  }
  function loadNavigation(){
    if(window.MarketBaseNavigation||document.querySelector('script[data-mb-navigation]'))return;
    var script=document.createElement('script');script.src=NAVIGATION_URL.href;script.async=false;script.dataset.mbNavigation='';document.head.appendChild(script);
  }
  function scheduleNavigation(){
    ensureNavigationStyles();
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadNavigation,{once:true});else loadNavigation();
  }
  function loadUpdateController(){
    if(window.MarketBaseUpdate||document.querySelector('script[data-mb-controller-fallback]'))return;
    var script=document.createElement('script');
    var retryUrl=new URL(UPDATE_CONTROLLER_URL.href);
    retryUrl.searchParams.set('retry',String(Date.now()));
    script.src=retryUrl.href;
    script.async=false;
    script.dataset.mbUpdateController='';
    script.dataset.mbControllerFallback='';
    document.head.appendChild(script);
  }
  function scheduleUpdateControllerFallback(){
    // Wait until parser-blocking footer scripts have either loaded or failed.
    // Loading here during parsing previously raced the explicit controller and
    // allowed a cached legacy controller to win on some pages.
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadUpdateController,{once:true});
    else window.setTimeout(loadUpdateController,0);
  }
  ...
  function goSafeBack(control){
    var target=safeBackTarget(control);if(!target)return false;
    try{
      var targetUrl=new URL(target);if(targetUrl.href===location.href)return false;
      var currentIsHome=normalizedIndexPath(location.pathname)===normalizedIndexPath(HOME_URL.pathname);
      var targetIsHome=normalizedIndexPath(targetUrl.pathname)===normalizedIndexPath(HOME_URL.pathname);
      if(currentIsHome&&targetIsHome&&window.MarketBaseInPageRouter){
        window.MarketBaseInPageRouter.navigate(targetUrl.searchParams.get('view')||'home',{source:'safe-back'});
        return true;
      }
    }catch(_e){return false;}
    window.location.replace(target);return true;
  }
  ...
  function mountScrollControls(){
    ...
    up.addEventListener('click',function(){window.scrollTo({top:0,left:0,behavior:scrollBehavior()});});back.addEventListener('click',function(){goSafeBack(back);});down.addEventListener('click',function(){var end=Math.max(document.documentElement.scrollHeight,document.body.scrollHeight);window.scrollTo({top:end,left:0,behavior:scrollBehavior()});});
  ...
  window.MarketBaseSafeBack=Object.freeze({target:safeBackTarget,go:goSafeBack});document.addEventListener('click',interceptPageBack,true);scheduleNavigation();scheduleUpdateControllerFallback();if(document.body)mountScrollControls();else document.addEventListener('DOMContentLoaded',mountScrollControls,{once:true});
})();
