  ...
  function stationCards(rows) {
    return rows.map(station => `
      <article class="station-card" data-station-card="${escapeHtml(station.id)}">
        ...
        <a class="radio-open" data-radio-online-only href="player.html?id=${encodeURIComponent(station.id)}&amp;autoplay=1&amp;v=20260803-v333-7-layout-nav-route-map-unification" target="_blank" rel="noopener">
