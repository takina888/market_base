(function (global) {
  ...
  function generationIsCurrent(generation) {
    return String(readState().generation || '') === String(generation || '');
  }

  function requireCurrentGeneration(generation) {
  ...
  function currentBuildId() {
    return global.MarketBaseUpdate?.buildId || global.MARKET_BASE_BUILD?.id || BUILD_ID;
  }
  ...
  async function notifyOfflineModeChanged(active, generation) {
    ...
    if (global.MarketBaseUpdate?.notifyOfflineModeChanged) {
      return global.MarketBaseUpdate.notifyOfflineModeChanged(active, generation);
    }
    if (!('serviceWorker' in navigator) || !('MessageChannel' in global)) {
      throw new Error('更新機能を読み込めません');
    }
    const registration = await navigator.serviceWorker.getRegistration?.(SITE_ROOT.href);
    requireCurrentGeneration(generation);
    if (!registration?.active) throw new Error('Service Workerを確認できません');
    const requestId = `${generation}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    return withTimeout(new Promise((resolve, reject) => {
      const channel = new MessageChannel();
      channel.port1.onmessage = event => {
        const reply = event.data || {};
        if (
          reply.type !== 'OFFLINE_MODE_CHANGED_ACK' ||
          reply.requestId !== requestId ||
          reply.ok !== true ||
          reply.active !== !!active ||
          String(reply.generation || '') !== String(generation || '')
        ) {
          reject(new Error('オフライン状態の確認に失敗しました'));
          return;
        }
        resolve(reply);
      };
      try {
        ...
        registration.active.postMessage({
          type: 'OFFLINE_MODE_CHANGED', requestId,
          active: !!active, generation: String(generation || '')
        }, [channel.port2]);
      } catch (error) {
        reject(error);
      }
    }), 2000, 'オフライン状態の確認がタイムアウトしました').then(reply => {
      requireCurrentGeneration(generation);
      return reply;
    });
  }
  ...
  function offlineSaveFetchUrl(input) {
    const url = new URL(input, SITE_ROOT);
    if (url.origin === SITE_ROOT.origin) {
      url.searchParams.set('mb-offline-save', '1');
    }
    return url.href;
  }
  ...
  async function enableOfflineSentinel(state, generation) {
    requireCurrentGeneration(generation);
  ...
  async function disableOfflineSentinel(generation) {
    requireCurrentGeneration(generation);
    await caches.delete(STATE_CACHE);
    requireCurrentGeneration(generation);
    // Saving/downloading starts only after the worker confirms its memo is no
    // longer serving the previous snapshot.
    await notifyOfflineModeChanged(false, generation);
  }

  async function cacheOneStaticAsset(cache, asset, generation) {
    const url = absoluteAsset(asset);
    const cached = await cache.match(url, { ignoreSearch: true });
    try {
      const response = await fetch(offlineSaveFetchUrl(url), {
        cache: 'reload',
  ...
  async function cacheStaticAssets(assets, generation) {
    ...
    async function worker() {
      while (queue.length) {
        ...
        const result = await cacheOneStaticAsset(cache, asset, generation);
        requireCurrentGeneration(generation);
  ...
  async function cacheOptionalAssets(assets, generation) {
    const cache = await caches.open(TEXT_CACHE);
    const queue = [...assets];
    let saved = 0;
    let completed = 0;
  ...
  async function cacheCompressedImages(urls, generation) {
    ...
    for (let index = 0; index < urls.length; index += 1) {
      ...
      try {
        ...
        if (error?.code === 'MB_STALE_OFFLINE_GENERATION') throw error;
  ...
  async function saveOffline() {
    ...
    try {
      ...
      const swUrl = new URL('../sw.js', global.location.href);
      ...
      await disableOfflineSentinel(generation);
      requireCurrentGeneration(generation);
      ...
      const textResult = await cacheStaticAssets(textAssets, generation);
      ...
      const supportResult = await cacheOptionalAssets(supportAssets, generation);
      requireCurrentGeneration(generation);
      writeState({
        enabled: true,
        phase: 'saving-images',
      ...
      const imageResult = await cacheCompressedImages(photoUrls, generation);
      requireCurrentGeneration(generation);
      ...
      await enableOfflineSentinel(finished, generation);
      ...
      try { await enableOfflineSentinel(partial, generation); } catch (_) {}
  ...
  async function finishOnlineTransition() {
    ...
    try {
      ...
      if (generation && !generationIsCurrent(generation)) return false;
      ...
      if (!generation || generationIsCurrent(generation)) {
        writeState({
          enabled: false,
          pendingCleanup: true,
